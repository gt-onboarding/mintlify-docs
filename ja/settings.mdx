---
title: "グローバル設定"
description: "`docs.json` ファイルでサイト全体の設定を行う"
icon: "settings-2"
keywords: ["docs.json", "設定", "カスタマイズ", "構成"]
---

import IconsOptional from "/snippets/ja/icons-optional.mdx";

`docs.json` ファイルを使うと、Markdown ファイルの集合を、ナビゲーション可能でカスタマイズされたドキュメントサイトに変換できます。必須のこの設定ファイルは、スタイル、ナビゲーション、連携機能などを管理します。ドキュメントの設計図と考えてください。

`docs.json` の設定は、すべてのページに対してグローバルに適用されます。

<div id="setting-up-your-docsjson">
  ## `docs.json` の設定
</div>

はじめるには、`theme`、`name`、`colors.primary`、`navigation` を指定するだけで十分です。その他のフィールドは任意で、ドキュメントの要件に応じて後から追加できます。

最良の編集体験のために、`docs.json` ファイルの先頭にスキーマ参照を記載してください。これにより、ほとんどのコードエディタでオートコンプリート、検証、ツールチップが有効になります。

```json
{
  "$schema": "https://mintlify.com/docs.json",
  "theme": "mint",
  "name": "Your Docs",
  "colors": {
    "primary": "#ff0000"
  },
  "navigation": {
    // ナビゲーション構造
  }
  // そのほかの設定
}
```

<div id="reference">
  ## リファレンス
</div>

このセクションでは、`docs.json` ファイルのリファレンスを全て掲載します。

<div id="customization">
  ### カスタマイズ
</div>

<ResponseField name="theme" required>
  サイトのレイアウトテーマ。

  次のいずれか: `mint`, `maple`, `palm`, `willow`, `linden`, `almond`, `aspen`。

  詳しくは[Themes](ja/themes)を参照してください。
</ResponseField>

<ResponseField name="name" type="string" required>
  プロジェクト、組織、またはプロダクトの名称。
</ResponseField>

<ResponseField name="colors" type="object" required>
  ドキュメントで使用するカラー。テーマによって適用方法が異なります。`primary` だけを指定した場合、すべてのカラ要素にその色が使われます。

  <Expandable title="Colors">
    <ResponseField name="primary" type="string matching ^#([a-fA-F0-9]{6}|[a-fA-F0-9]{3})$" required>
      ドキュメントの主要カラー。一般的にライトモードでの強調に使用されます（テーマにより多少異なります）。

      `#` で始まる16進数コードである必要があります。
    </ResponseField>

    <ResponseField name="light" type="string matching ^#([a-fA-F0-9]{6}|[a-fA-F0-9]{3})$">
      ダークモードでの強調に使用するカラー。

      `#` で始まる16進数コードである必要があります。
    </ResponseField>

    <ResponseField name="dark" type="string matching ^#([a-fA-F0-9]{6}|[a-fA-F0-9]{3})$">
      ライト/ダーク両モードで、ボタンやホバー状態に使用されるカラー（テーマにより多少異なります）。

      `#` で始まる16進数コードである必要があります。
    </ResponseField>
  </Expandable>
</ResponseField>

<ResponseField name="description" type="string">
  SEO と AI インデックス用のサイト説明。
</ResponseField>

<ResponseField name="logo" type="string or object">
  ライトモードとダークモードの両方で使用するロゴ。

  <Expandable title="Logo">
    <ResponseField name="light" type="string" required>
      ライトモード用ロゴファイルへのパス。拡張子を含めてください。例: `/logo.png`
    </ResponseField>

    <ResponseField name="dark" type="string" required>
      ダークモード用ロゴファイルへのパス。拡張子を含めてください。例: `/logo-dark.png`
    </ResponseField>

    <ResponseField name="href" type="string (uri)">
      ロゴをクリックしたときに遷移するURL。未指定の場合はホームページにリンクします。例: `https://mintlify.com`
    </ResponseField>
  </Expandable>
</ResponseField>

<ResponseField name="favicon" type="string or object">
  ファビコンファイルへのパス（拡張子を含む）。適切なサイズに自動リサイズされます。
  単一ファイル、またはライト/ダークで別ファイルを指定できます。例: `/favicon.png`

  <Expandable title="Favicon">
    <ResponseField name="light" type="string" required>
      ライトモード用ファビコンファイルへのパス。拡張子を含めてください。例: `/favicon.png`
    </ResponseField>

    <ResponseField name="dark" type="string" required>
      ダークモード用ファビコンファイルへのパス。拡張子を含めてください。例: `/favicon-dark.png`
    </ResponseField>
  </Expandable>
</ResponseField>

<ResponseField name="thumbnails" type="object">
  ソーシャルメディアおよびページプレビュー向けのサムネイル設定。

  <Expandable title="Thumbnails">
    <ResponseField name="appearance" type="&#x22;light&#x22; | &#x22;dark&#x22;">
      サムネイルのビジュアルテーマ。未指定の場合、`colors` フィールドで定義されたサイトのカラースキームが使用されます。
    </ResponseField>

    <ResponseField name="background" type="string">
      サムネイルの背景画像。相対パスまたは絶対URLを指定できます。
    </ResponseField>
  </Expandable>
</ResponseField>

<ResponseField name="styling" type="object">
  ビジュアルスタイリングの設定。

  <Expandable title="Styling">
    <ResponseField name="eyebrows" type="&#x22;section&#x22; | &#x22;breadcrumbs&#x22;">
      ページのアイブロウのスタイル。セクション名を表示する場合は `section`、完全なナビゲーションパスを表示する場合は `breadcrumbs` を選択します。デフォルトは `section`。
    </ResponseField>

    <ResponseField name="codeblocks" type="&#x22;system&#x22; | &#x22;dark&#x22;">
      コードブロックのテーマ。サイトテーマに合わせる場合は `system`、常にダークテーマにする場合は `dark` を選択します。デフォルトは `system`。
    </ResponseField>
  </Expandable>
</ResponseField>

<ResponseField name="icons" type="object">
  アイコンライブラリの設定。

  <Expandable title="Icons">
    <ResponseField name="library" type="&#x22;fontawesome&#x22; | &#x22;lucide&#x22;" required>
      ドキュメント全体で使用するアイコンライブラリ。デフォルトは `fontawesome` です。

      <Note>
        ライブラリ設定に関係なく、各アイコンごとに外部ホストのアイコンURL、プロジェクト内のアイコンファイルへのパス、または JSX 互換の SVG コードを指定できます。
      </Note>
    </ResponseField>
  </Expandable>
</ResponseField>

<ResponseField name="fonts" type="object">
  ドキュメントのフォント設定。デフォルトのフォントは [Inter](https://fonts.google.com/specimen/Inter) です。

  <Expandable title="Fonts">
    <ResponseField name="family" type="string" required>
      フォントファミリー（例: &quot;Open Sans&quot;, &quot;Playfair Display&quot;）。
    </ResponseField>

    <ResponseField name="weight" type="number">
      フォントの太さ（例: 400 や 700）。可変フォントは 550 のような細かな指定もサポートします。
    </ResponseField>

    <ResponseField name="source" type="string (uri)">
      フォントソースの URL（例: https://mintlify-assets.b-cdn.net/fonts/Hubot-Sans.woff2）。[Google Fonts](https://fonts.google.com) は、Google Font の `family` 名を指定すると自動で読み込まれるため、ソース URL は不要です。
    </ResponseField>

    <ResponseField name="format" type="&#x22;woff&#x22; | &#x22;woff2&#x22;">
      フォントファイル形式。
    </ResponseField>

    <ResponseField name="heading" type="object">
      見出し用のフォント設定を上書きします。

      <Expandable title="Heading">
        <ResponseField name="family" type="string" required>
          フォントファミリー（例: &quot;Open Sans&quot;, &quot;Playfair Display&quot;）。
        </ResponseField>

        <ResponseField name="weight" type="number">
          フォントの太さ（例: 400, 700）。可変フォントは 550 のような細かな指定もサポートします。
        </ResponseField>

        <ResponseField name="source" type="string (uri)">
          フォントソースの URL（例: https://mintlify-assets.b-cdn.net/fonts/Hubot-Sans.woff2）。[Google Fonts](https://fonts.google.com) は、Google Font の `family` 名を指定すると自動で読み込まれるため、ソース URL は不要です。
        </ResponseField>

        <ResponseField name="format" type="&#x22;woff&#x22; | &#x22;woff2&#x22;">
          フォントファイル形式。
        </ResponseField>
      </Expandable>
    </ResponseField>

    <ResponseField name="body" type="object">
      本文用のフォント設定を上書きします。

      <Expandable title="Body">
        <ResponseField name="family" type="string" required>
          フォントファミリー（例: &quot;Open Sans&quot;, &quot;Playfair Display&quot;）。
        </ResponseField>

        <ResponseField name="weight" type="number">
          フォントの太さ（例: 400, 700）。可変フォントは 550 のような細かな指定もサポートします。
        </ResponseField>

        <ResponseField name="source" type="string (uri)">
          フォントソースの URL（例: https://mintlify-assets.b-cdn.net/fonts/Hubot-Sans.woff2）。[Google Fonts](https://fonts.google.com) は、Google Font の `family` 名を指定すると自動で読み込まれるため、ソース URL は不要です。
        </ResponseField>

        <ResponseField name="format" type="&#x22;woff&#x22; | &#x22;woff2&#x22;">
          フォントファイル形式。
        </ResponseField>
      </Expandable>
    </ResponseField>
  </Expandable>
</ResponseField>

<ResponseField name="appearance" type="object">
  ライト/ダークモードの切り替え設定。

  <Expandable title="Appearance">
    <ResponseField name="default" type="&#x22;system&#x22; | &#x22;light&#x22; | &#x22;dark&#x22;">
      デフォルトのテーマモード。ユーザーの OS 設定に合わせるには `system`、特定のモードを固定するには `light` または `dark` を選択します。デフォルトは `system` です。
    </ResponseField>

    <ResponseField name="strict" type="boolean">
      ライト/ダークモードの切り替えを非表示にするかどうか。デフォルトは `true` です。
    </ResponseField>
  </Expandable>
</ResponseField>

<ResponseField name="background" type="object">
  背景色と装飾の設定。

  <Expandable title="Background">
    <ResponseField name="image" type="string or object">
      サイトの背景画像。単一ファイル、またはライトモードとダークモードで別々のファイルを指定できます。

      <Expandable title="Image">
        <ResponseField name="light" type="string" required>
          ライトモード用の背景画像のパス。拡張子を含めてください。例: `/background.png`。
        </ResponseField>

        <ResponseField name="dark" type="string" required>
          ダークモード用の背景画像のパス。拡張子を含めてください。例: `/background-dark.png`。
        </ResponseField>
      </Expandable>
    </ResponseField>

    <ResponseField name="decoration" type="&#x22;gradient&#x22; | &#x22;grid&#x22; | &#x22;windows&#x22;">
      テーマの背景装飾。
    </ResponseField>

    <ResponseField name="color" type="object">
      ライトモードとダークモード用のカスタム背景色。

      <Expandable title="Color">
        <ResponseField name="light" type="string matching ^#([a-fA-F0-9]{6}|[a-fA-F0-9]{3})$">
          ライトモードの背景色。

          `#` で始まる 16 進数コードである必要があります。
        </ResponseField>

        <ResponseField name="dark" type="string matching ^#([a-fA-F0-9]{6}|[a-fA-F0-9]{3})$">
          ダークモードの背景色。

          `#` で始まる 16 進数コードである必要があります。
        </ResponseField>
      </Expandable>
    </ResponseField>
  </Expandable>
</ResponseField>

<div id="structure">
  ### 構造
</div>

<ResponseField name="navbar" type="object">
  ナビゲーションバーの項目。

  <Expandable title="Navbar">
    <ResponseField name="links" type="array of object">
      ナビゲーションバーに表示するリンク。

      <Expandable title="Links">
        <ResponseField name="label" type="string" required>
          リンクのテキスト。
        </ResponseField>

        <ResponseField name="href" type="string (uri)" required>
          リンク先のURLまたはパス。
        </ResponseField>

        <IconsOptional />
      </Expandable>
    </ResponseField>

    <ResponseField name="primary" type="object">
      ナビゲーションバーのプライマリボタン。

      <Expandable title="Primary">
        <ResponseField name="type" type="&#x22;button&#x22; | &#x22;github&#x22;" required>
          ボタンのスタイル。ラベル付きの標準ボタンには`button`、GitHubリポジトリへのリンク（アイコン付き）には`github`を選択します。
        </ResponseField>

        <ResponseField name="label" type="string" required>
          ボタンのテキスト。`type`が`button`の場合にのみ有効です。
        </ResponseField>

        <ResponseField name="href" type="string (uri)" required>
          ボタンの遷移先。有効なパスまたは外部URLである必要があります。`type`が`github`の場合は、GitHubリポジトリのURLである必要があります。
        </ResponseField>
      </Expandable>
    </ResponseField>
  </Expandable>
</ResponseField>

<ResponseField name="navigation" type="object" required>
  コンテンツのナビゲーション構造。

  <Expandable title="Navigation">
    <ResponseField name="global" type="object">
      すべてのページやセクションで共通して表示されるグローバルナビゲーション要素。

      <Expandable title="Global">
        <ResponseField name="languages" type="array of object">
          ローカライズサイト向けの言語スイッチャー設定。

          <Expandable title="Languages">
            <ResponseField name="language" type="&#x22;en&#x22; | &#x22;cn&#x22; | &#x22;zh&#x22; | &#x22;zh-Hans&#x22; | &#x22;zh-Hant&#x22; | &#x22;es&#x22; | &#x22;fr&#x22; | &#x22;ja&#x22; | &#x22;jp&#x22; | &#x22;pt&#x22; | &#x22;pt-BR&#x22; | &#x22;de&#x22; | &#x22;ko&#x22; | &#x22;it&#x22; | &#x22;ru&#x22; | &#x22;id&#x22; | &#x22;ar&#x22; | &#x22;tr&#x22;" required>
              ISO 639-1 形式の言語コード
            </ResponseField>

            <ResponseField name="default" type="boolean">
              これを既定の言語にするかどうか。
            </ResponseField>

            <ResponseField name="hidden" type="boolean">
              この言語オプションを既定で非表示にするかどうか。
            </ResponseField>

            <ResponseField name="href" type="string (uri)" required>
              該当言語版ドキュメントへの有効なパスまたは外部リンク。
            </ResponseField>
          </Expandable>
        </ResponseField>

        <ResponseField name="versions" type="array of object">
          マルチバージョンサイト向けのバージョンスイッチャーの設定。

          <Expandable title="Versions">
            <ResponseField name="version" type="string" required>
              バージョンの表示名。

              最小文字数: 1
            </ResponseField>

            <ResponseField name="default" type="boolean">
              これがデフォルトのバージョンかどうか。
            </ResponseField>

            <ResponseField name="hidden" type="boolean">
              既定でこのバージョンの選択肢を非表示にするかどうか。
            </ResponseField>

            <ResponseField name="href" type="string (uri)" required>
              このバージョンのドキュメントへのURLまたはパス。
            </ResponseField>
          </Expandable>
        </ResponseField>

        <ResponseField name="tabs" type="array of object">
          主要セクションを整理するためのトップレベルのナビゲーションタブ。

          <Expandable title="Tabs">
            <ResponseField name="tab" type="string" required>
              タブの表示名。

              最小文字数: 1
            </ResponseField>

            <IconsOptional />

            <ResponseField name="hidden" type="boolean">
              既定でこのタブを非表示にするかどうか。
            </ResponseField>

            <ResponseField name="href" type="string (uri)" required>
              タブのリンク先のURLまたはパス。
            </ResponseField>
          </Expandable>
        </ResponseField>

        <ResponseField name="anchors" type="array of object">
          サイドバーのナビゲーションで目立つ位置に表示されるアンカーリンク。

          <Expandable title="Anchors">
            <ResponseField name="anchor" type="string" required>
              アンカーの表示名。

              最小文字数: 1
            </ResponseField>

            <IconsOptional />

            <ResponseField name="color" type="object">
              アンカー用のカスタムカラー。

              <Expandable title="Color">
                <ResponseField name="light" type="string matching ^#([a-fA-F0-9]{6}|[a-fA-F0-9]{3})$">
                  ライトモード用のアンカーカラー。

                  「#」で始まる16進数コードである必要があります。
                </ResponseField>

                <ResponseField name="dark" type="string matching ^#([a-fA-F0-9]{6}|[a-fA-F0-9]{3})$">
                  ダークモード用のアンカーカラー。

                  「#」で始まる16進数コードである必要があります。
                </ResponseField>
              </Expandable>
            </ResponseField>

            <ResponseField name="hidden" type="boolean">
              既定でこのアンカーを非表示にするかどうか。
            </ResponseField>

            <ResponseField name="href" type="string (uri)" required>
              アンカーのリンク先のURLまたはパス。
            </ResponseField>
          </Expandable>
        </ResponseField>

        <ResponseField name="dropdowns" type="array of object">
          関連コンテンツを整理するためのドロップダウンメニュー。

          <Expandable title="Dropdowns">
            <ResponseField name="dropdown" type="string" required>
              ドロップダウンの表示名。

              最小長: 1
            </ResponseField>

            <IconsOptional />

            <ResponseField name="hidden" type="boolean">
              既定でこのドロップダウンを非表示にするかどうか。
            </ResponseField>

            <ResponseField name="href" type="string (uri)" required>
              ドロップダウン先のURLまたはパス。
            </ResponseField>
          </Expandable>
        </ResponseField>
      </Expandable>
    </ResponseField>

    <ResponseField name="languages" type="array of object">
      [multi-language](ja/navigation#languages) サイト用の言語切り替え。
    </ResponseField>

    <ResponseField name="versions" type="array of object">
      複数の[バージョン](ja/navigation#versions)があるサイト向けのバージョンスイッチャー。
    </ResponseField>

    <ResponseField name="tabs" type="array of object">
      トップレベルのナビゲーションの[タブ](ja/navigation#tabs)。
    </ResponseField>

    <ResponseField name="anchors" type="array of object">
      サイドバーの[アンカー](ja/navigation#anchors)
    </ResponseField>

    <ResponseField name="dropdowns" type="array of object">
      関連コンテンツをまとめるための[ドロップダウン](ja/navigation#dropdowns)。
    </ResponseField>

    <ResponseField name="groups" type="array of object">
      [グループ](ja/navigation#groups) でコンテンツをセクションに整理します。
    </ResponseField>

    <ResponseField name="pages" type="array of string or object">
      ドキュメントを構成する個々の[ページ](ja/navigation#pages)。
    </ResponseField>
  </Expandable>
</ResponseField>

<ResponseField name="interaction" type="object">
  ナビゲーション要素のユーザー操作設定。

  <Expandable title="インタラクション">
    <ResponseField name="drilldown" type="boolean">
      ナビゲーショングループ選択時の自動遷移の挙動を制御します。グループを展開した際に最初のページへ自動で移動させるには `true` を設定します。遷移せずにグループの展開/折りたみのみを行うには `false` を設定します。未設定の場合はテーマのデフォルト動作が適用されます。
    </ResponseField>
  </Expandable>
</ResponseField>

<ResponseField name="footer" type="object">
  フッターのコンテンツとソーシャルメディアリンク。

  <Expandable title="フッター">
    <ResponseField name="socials" type="object">
      フッターに表示するソーシャルメディアのプロフィール。各キーはプラットフォーム名、各値はプロフィールのURLです。例:

      ```json
      {
        "x": "https://x.com/mintlify"
      }
      ```

      使用可能なプロパティ名: `x`, `website`, `facebook`, `youtube`, `discord`, `slack`, `github`, `linkedin`, `instagram`, `hacker-news`, `medium`, `telegram`, `twitter`, `x-twitter`, `earth-americas`, `bluesky`, `threads`, `reddit`, `podcast`
    </ResponseField>

    <ResponseField name="links" type="array of object">
      フッターに表示するリンク。

      <Expandable title="リンク">
        <ResponseField name="header" type="string">
          列のヘッダータイトル。

          最小文字数: 1
        </ResponseField>

        <ResponseField name="items" type="array of object" required>
          列に表示するリンク。

          <Expandable title="アイテム">
            <ResponseField name="label" type="string" required>
              リンクテキスト。

              最小文字数: 1
            </ResponseField>

            <ResponseField name="href" type="string (uri)" required>
              リンク先のURL。
            </ResponseField>
          </Expandable>
        </ResponseField>
      </Expandable>
    </ResponseField>
  </Expandable>
</ResponseField>

<ResponseField name="banner" type="object">
  サイト全体のページ上部に表示されるバナー。

  <Expandable title="バナー">
    <ResponseField name="content" type="string">
      バナーの内容。プレーンテキストとMarkdownの書式に対応します。例:

      ```json
      {
        "content": "🚀 バナーを公開しました！ [詳細はこちら](mintlify.com)"
      }
      ```
    </ResponseField>

    <ResponseField name="dismissible" type="boolean">
      ユーザーがバナーを閉じられるかどうか。デフォルトは `false`。
    </ResponseField>
  </Expandable>
</ResponseField>

<ResponseField name="redirects" type="array of object">
  移動・名称変更・削除されたページのためのリダイレクト。

  <Expandable title="リダイレクト">
    <ResponseField name="source" type="string" required>
      リダイレクト元のパス。例: `/old-page`
    </ResponseField>

    <ResponseField name="destination" type="string" required>
      リダイレクト先のパス。例: `/new-page`
    </ResponseField>

    <ResponseField name="permanent" type="boolean">
      永続的なリダイレクト（301）を使用するかどうか。デフォルトは `true`。
    </ResponseField>
  </Expandable>
</ResponseField>

<ResponseField name="contextual" type="object">
  AI最適化されたコンテンツと統合のためのコンテキストメニュー。

  <Expandable title="コンテキスト">
    <ResponseField name="options" type="array of &#x22;copy&#x22; | &#x22;view&#x22; | &#x22;chatgpt&#x22; | &#x22;claude&#x22; | &#x22;perplexity&#x22; | &#x22;mcp&#x22; | &#x22;cursor&#x22; | &#x22;vscode&#x22;" required>
      コンテキストメニューで利用可能なアクション。最初のオプションがデフォルトとして表示されます。

      * `copy`: 現在のページをMarkdownとしてクリップボードにコピーします。
      * `view`: 現在のページをMarkdownとして新しいタブで表示します。
      * `chatgpt`: 現在のページの内容をChatGPTに送信します。
      * `claude`: 現在のページの内容をClaudeに送信します。
      * `perplexity`: 現在のページの内容をPerplexityに送信します。
      * `mcp`: MCPサーバーのURLをクリップボードにコピーします。
      * `cursor`: ホスト済みMCPサーバーをCursorにインストールします。
      * `vscode`: ホスト済みMCPサーバーをVSCodeにインストールします。

      <img src="/images/page-context-menu.png" alt="Contextual Menu" className="rounded-xl" />

      <Note>
        コンテキストメニューはプレビューおよび本番デプロイでのみ利用可能です。
      </Note>
    </ResponseField>
  </Expandable>
</ResponseField>

<div id="api-configurations">
  ### API 設定
</div>

<ResponseField name="api" type="object">
  API ドキュメントとインタラクティブなプレイグラウンドの設定。

  <Expandable title="Api">
    <ResponseField name="openapi" type="string or array or object">
      API ドキュメントを生成するための OpenAPI 仕様ファイル。単一の URL/パス、または URL/パスの配列を指定できます。

      <Expandable title="Openapi">
        <ResponseField name="source" type="string">
          OpenAPI 仕様ファイルへの URL またはパス。

          最小文字数: 1
        </ResponseField>

        <ResponseField name="directory" type="string">
          OpenAPI ファイルを検索するディレクトリ。

          先頭にスラッシュを付けないでください。
        </ResponseField>
      </Expandable>
    </ResponseField>

    <ResponseField name="asyncapi" type="string or array or object">
      API ドキュメントを生成するための AsyncAPI 仕様ファイル。単一の URL/パス、または URL/パスの配列を指定できます。

      <Expandable title="Asyncapi">
        <ResponseField name="source" type="string">
          AsyncAPI 仕様ファイルへの URL またはパス。

          最小文字数: 1
        </ResponseField>

        <ResponseField name="directory" type="string">
          AsyncAPI ファイルを検索するディレクトリ。

          先頭にスラッシュを付けないでください。
        </ResponseField>
      </Expandable>
    </ResponseField>

    <ResponseField name="params" type="object">
      API パラメータの表示設定。

      <Expandable title="Params">
        <ResponseField name="expanded" type="&#x22;all&#x22; | &#x22;closed&#x22;">
          既定で全パラメータを展開するかどうか。既定値は `closed` です。
        </ResponseField>
      </Expandable>
    </ResponseField>

    <ResponseField name="playground" type="object">
      API プレイグラウンドの設定。

      <Expandable title="Playground">
        <ResponseField name="display" type="&#x22;interactive&#x22; | &#x22;simple&#x22; | &#x22;none&#x22;">
          API プレイグラウンドの表示モード。既定値は `interactive` です。
        </ResponseField>

        <ResponseField name="proxy" type="boolean">
          API リクエストをプロキシサーバー経由で送信するかどうか。既定値は `true` です。
        </ResponseField>
      </Expandable>
    </ResponseField>

    <ResponseField name="examples" type="object">
      自動生成される API サンプルの設定。

      <Expandable title="Examples">
        <ResponseField name="languages" type="array of string">
          自動生成される API スニペットのサンプル言語
        </ResponseField>

        <ResponseField name="defaults" type="&#x22;required&#x22; | &#x22;all&#x22;">
          API サンプルで任意パラメータを表示するかどうか。既定値は `all` です。
        </ResponseField>
      </Expandable>
    </ResponseField>

    <ResponseField name="mdx" type="object">
      `MDX` ファイルから生成される API ページの設定。

      <Expandable title="Mdx">
        <ResponseField name="auth" type="object">
          MDX ベースの API リクエストの認証設定。

          <Expandable title="Auth">
            <ResponseField name="method" type="&#x22;bearer&#x22; | &#x22;basic&#x22; | &#x22;key&#x22; | &#x22;cobo&#x22;">
              API リクエストの認証方式。
            </ResponseField>

            <ResponseField name="name" type="string">
              API リクエストの認証名。
            </ResponseField>
          </Expandable>
        </ResponseField>

        <ResponseField name="server" type="string or array">
          API リクエストのサーバー設定。
        </ResponseField>
      </Expandable>
    </ResponseField>
  </Expandable>
</ResponseField>

<div id="seo-and-search">
  ### SEO と検索
</div>

<ResponseField name="seo" type="object">
  SEO のインデックス設定。

  <Expandable title="Seo">
    <ResponseField name="metatags" type="object">
      すべてのページに追加されるメタタグ。有効なキーと値のペアである必要があります。利用可能なオプションは [common meta tags reference](/ja/settings/seo#common-meta-tags-reference) を参照してください。
    </ResponseField>

    <ResponseField name="indexing" type="&#x22;navigable&#x22; | &#x22;all&#x22;">
      検索エンジンにインデックスさせるページを指定します。`navigable` は `docs.json` のナビゲーションに含まれるページのみを、`all` はすべてのページをインデックスします。デフォルトは `navigable` です。
    </ResponseField>
  </Expandable>
</ResponseField>

<ResponseField name="search" type="object">
  検索の表示設定。

  <Expandable title="Search">
    <ResponseField name="prompt" type="string">
      検索バーに表示するプレースホルダーテキスト。
    </ResponseField>
  </Expandable>
</ResponseField>

<div id="integrations">
  ### 連携機能
</div>

<ResponseField name="integrations" type="object">
  サードパーティ連携

  <Expandable title="Integrations">
    <ResponseField name="amplitude" type="object">
      Amplitude のアナリティクス統合。

      <Expandable title="Amplitude">
        <ResponseField name="apiKey" type="string" required>
          Amplitude の API キー。
        </ResponseField>
      </Expandable>
    </ResponseField>

    <ResponseField name="clearbit" type="object">
      Clearbit のデータエンリッチメント統合。

      <Expandable title="Clearbit">
        <ResponseField name="publicApiKey" type="string" required>
          Clearbit の API キー。
        </ResponseField>
      </Expandable>
    </ResponseField>

    <ResponseField name="fathom" type="object">
      Fathom のアナリティクス連携。

      <Expandable title="Fathom">
        <ResponseField name="siteId" type="string" required>
          Fathom のサイト ID。
        </ResponseField>
      </Expandable>
    </ResponseField>

    <ResponseField name="frontchat" type="object">
      Frontチャットの連携。

      <Expandable title="Frontchat">
        <ResponseField name="snippetId" type="string" required>
          FrontチャットのスニペットID。

          最小長: 6
        </ResponseField>
      </Expandable>
    </ResponseField>

    <ResponseField name="ga4" type="object">
      Google Analytics 4 の連携。

      <Expandable title="GA4">
        <ResponseField name="measurementId" type="string matching ^G" required>
          Google Analytics 4 の測定 ID。

          次のパターンに一致する必要があります: ^G
        </ResponseField>
      </Expandable>
    </ResponseField>

    <ResponseField name="gtm" type="object">
      Google Tag Manager のインテグレーション。

      <Expandable title="Gtm">
        <ResponseField name="tagId" type="string matching ^G" required>
          Google Tag Manager のタグ ID。

          次のパターンに一致する必要があります: ^G
        </ResponseField>
      </Expandable>
    </ResponseField>

    <ResponseField name="heap" type="object">
      Heap のアナリティクス連携。

      <Expandable title="Heap">
        <ResponseField name="appId" type="string" required>
          Heap のアプリ ID。
        </ResponseField>
      </Expandable>
    </ResponseField>

    <ResponseField name="hotjar" type="object">
      Hotjar のインテグレーション。

      <Expandable title="Hotjar">
        <ResponseField name="hjid" type="string" required>
          Hotjar の ID。
        </ResponseField>

        <ResponseField name="hjsv" type="string" required>
          Hotjar のスクリプトバージョン。
        </ResponseField>
      </Expandable>
    </ResponseField>

    <ResponseField name="intercom" type="object">
      Intercom 連携。

      <Expandable title="Intercom">
        <ResponseField name="appId" type="string" required>
          Intercom のアプリID。

          最小文字数: 6
        </ResponseField>
      </Expandable>
    </ResponseField>

    <ResponseField name="koala" type="object">
      Koala 連携。

      <Expandable title="Koala">
        <ResponseField name="publicApiKey" type="string" required>
          Koala のパブリック API キー。

          最小文字数: 2
        </ResponseField>
      </Expandable>
    </ResponseField>

    <ResponseField name="logrocket" type="object">
      LogRocket との連携。

      <Expandable title="LogRocket">
        <ResponseField name="appId" type="string" required>
          LogRocket のアプリID。
        </ResponseField>
      </Expandable>
    </ResponseField>

    <ResponseField name="mixpanel" type="object">
      Mixpanel のインテグレーション。

      <Expandable title="Mixpanel">
        <ResponseField name="projectToken" type="string" required>
          Mixpanel のプロジェクトトークン。
        </ResponseField>
      </Expandable>
    </ResponseField>

    <ResponseField name="osano" type="object">
      Osano 連携。

      <Expandable title="Osano">
        <ResponseField name="scriptSource" type="string" required>
          Osano スクリプトのソース。
        </ResponseField>
      </Expandable>
    </ResponseField>

    <ResponseField name="pirsch" type="object">
      Pirsch アナリティクスの統合。

      <Expandable title="Pirsch">
        <ResponseField name="id" type="string" required>
          Pirsch の ID。
        </ResponseField>
      </Expandable>
    </ResponseField>

    <ResponseField name="posthog" type="object">
      PostHog 連携。

      <Expandable title="Posthog">
        <ResponseField name="apiKey" type="string matching ^phc\_" required>
          PostHog の API キー。

          次のパターンに一致する必要があります: ^phc&#95;
        </ResponseField>

        <ResponseField name="apiHost" type="string (uri)">
          PostHog の API ホスト。
        </ResponseField>
      </Expandable>
    </ResponseField>

    <ResponseField name="plausible" type="object">
      Plausible のアナリティクス連携。

      <Expandable title="Plausible">
        <ResponseField name="domain" type="string" required>
          Plausible のドメイン。
        </ResponseField>

        <ResponseField name="server" type="string">
          Plausible のサーバー。
        </ResponseField>
      </Expandable>
    </ResponseField>

    <ResponseField name="segment" type="object">
      Segment 連携

      <Expandable title="Segment">
        <ResponseField name="key" type="string" required>
          Segment のキー。
        </ResponseField>
      </Expandable>
    </ResponseField>

    <ResponseField name="telemetry" type="object">
      テレメトリー設定

      <Expandable title="Telemetry">
        <ResponseField name="enabled" type="boolean">
          テレメトリーを有効にするかどうか
        </ResponseField>
      </Expandable>
    </ResponseField>

    <ResponseField name="cookies" type="object">
      Cookie 設定。

      <Expandable title="Cookies">
        <ResponseField name="key" type="string">
          Cookie のキー。
        </ResponseField>

        <ResponseField name="value" type="string">
          Cookie の値。
        </ResponseField>
      </Expandable>
    </ResponseField>
  </Expandable>
</ResponseField>

<div id="errors">
  ### エラー
</div>

<ResponseField name="errors" type="object">
  エラー処理の設定。

  <Expandable title="Errors">
    <ResponseField name="404" type="object">
      404「ページが見つかりません」エラーの処理。

      <Expandable title="404">
        <ResponseField name="redirect" type="boolean">
          ページが見つからない場合にホームページへ自動的にリダイレクトするかどうか。
        </ResponseField>

        <ResponseField name="title" type="string">
          404エラーページのカスタムタイトル。
        </ResponseField>

        <ResponseField name="description" type="string">
          404エラーページのカスタム説明。Markdownの書式をサポートします。
        </ResponseField>
      </Expandable>
    </ResponseField>
  </Expandable>
</ResponseField>

<div id="examples">
  ## 例
</div>

<Tabs>
  <Tab title="Basic example">
    ```json title="docs.json" wrap lines
    {
      "$schema": "https://mintlify.com/docs.json",
      "theme": "maple",
      "name": "Example Co.",
      "description": "Example Co. は、サンプルコンテンツとプレースホルダーテキストを提供する企業です。",
      "colors": {
        "primary": "#3B82F6",
        "light": "#F8FAFC",
        "dark": "#0F172A"
      },
      "navigation": {
        "dropdowns": [
          {
            "dropdown": "ドキュメント",
            "icon": "book",
            "description": "Example Co. のプロダクトの使い方",
            "groups": [
              {
                "group": "はじめに",
                "pages": [
                  "index",
                  "quickstart"
                ]
              },
              {
                "group": "カスタマイズ",
                "pages": [
                  "settings",
                  "users",
                  "features"
                ]
              },
              {
                "group": "請求",
                "pages": [
                  "billing/overview",
                  "billing/payments",
                  "billing/subscriptions"
                ]
              }
            ]
          },
          {
            "dropdown": "変更履歴",
            "icon": "history",
            "description": "更新内容と変更点",
            "pages": [
              "changelog"
            ]
          }
        ]
      },
      "logo": {
        "light": "/logo-light.svg",
        "dark": "/logo-dark.svg",
        "href": "https://example.com"
      },
      "navbar": {
        "links": [
          {
            "label": "コミュニティ",
            "href": "https://example.com/community"
          }
        ],
        "primary": {
          "type": "button",
          "label": "はじめる",
          "href": "https://example.com/start"
        }
      },
      "footer": {
        "socials": {
          "x": "https://x.com/example",
          "linkedin": "https://www.linkedin.com/company/example",
          "github": "https://github.com/example",
          "slack": "https://example.com/community"
        },
        "links": [
          {
            "header": "リソース",
            "items": [
              {
                "label": "導入事例",
                "href": "https://example.com/customers"
              },
              {
                "label": "エンタープライズ",
                "href": "https://example.com/enterprise"
              },
              {
                "label": "プレビューのリクエスト",
                "href": "https://example.com/preview"
              }
            ]
          },
          {
            "header": "会社情報",
            "items": [
              {
                "label": "採用情報",
                "href": "https://example.com/careers"
              },
              {
                "label": "ブログ",
                "href": "https://example.com/blog"
              },
              {
                "label": "プライバシーポリシー",
                "href": "https://example.com/legal/privacy"
              }
            ]
          }
        ]
      },
      "integrations": {
        "ga4": {
          "measurementId": "G-XXXXXXXXXX"
        },
        "koala": {
          "publicApiKey": "pk_example_key_123"
        },
        "telemetry": {
          "enabled": true
        },
        "cookies": {
          "key": "example_cookie_key",
          "value": "example_cookie_value"
        }
      },
      "contextual": {
        "options": [
          "copy",
          "view",
          "chatgpt",
          "claude"
        ]
      },
      "errors": {
        "404": {
          "redirect": false,
          "title": "見つかりません",
          "description": "この _ページ_ にいったい何が**起きた**のでしょう？"
        }
      }
    }
    ```
  </Tab>

  <Tab title="Interactive API example">
    ```json title="docs.json" wrap lines highlight={43-61, 72-79}
    {
      "$schema": "https://mintlify.com/docs.json",
      "theme": "maple",
      "name": "Example Co.",
      "description": "Example Co. は、サンプルコンテンツとプレースホルダーテキストを提供する企業です。",
      "colors": {
        "primary": "#3B82F6",
        "light": "#F8FAFC",
        "dark": "#0F172A"
      },
      "navigation": {
        "dropdowns": [
          {
            "dropdown": "Documentation",
            "icon": "book",
            "description": "Example Co. 製品の使い方",
            "groups": [
              {
                "group": "Getting started",
                "pages": [
                  "index",
                  "quickstart"
                ]
              },
              {
                "group": "Customization",
                "pages": [
                  "settings",
                  "users",
                  "features"
                ]
              },
              {
                "group": "Billing",
                "pages": [
                  "billing/overview",
                  "billing/payments",
                  "billing/subscriptions"
                ]
              }
            ]
          },
          {
            "dropdown": "API reference",
            "icon": "terminal",
            "description": "Example Co. API の使い方",
            "groups": [
              {
                "group": "API reference",
                "pages": [
                  "api-reference/introduction"
                ]
              },
              {
                "group": "Endpoints",
                "openapi": {
                  "source": "openapi.json"
                }
              }
            ]
          },
          {
            "dropdown": "Changelog",
            "icon": "history",
            "description": "更新情報",
            "pages": [
              "changelog"
            ]
          }
        ]
      },
      "api": {
        "playground": {
          "display": "interactive"
        },
        "examples": {
          "languages": ["javascript", "curl", "python"]
        }
      },
      "logo": {
        "light": "/logo-light.svg",
        "dark": "/logo-dark.svg",
        "href": "https://example.com"
      },
      "navbar": {
        "links": [
          {
            "label": "Community",
            "href": "https://example.com/community"
          }
        ],
        "primary": {
          "type": "button",
          "label": "はじめる",
          "href": "https://example.com/start"
        }
      },
      "footer": {
        "socials": {
          "x": "https://x.com/example",
          "linkedin": "https://www.linkedin.com/company/example",
          "github": "https://github.com/example",
          "slack": "https://example.com/community"
        },
        "links": [
          {
            "header": "Resources",
            "items": [
              {
                "label": "Customers",
                "href": "https://example.com/customers"
              },
              {
                "label": "Enterprise",
                "href": "https://example.com/enterprise"
              },
              {
                "label": "Request Preview",
                "href": "https://example.com/preview"
              }
            ]
          },
          {
            "header": "Company",
            "items": [
              {
                "label": "Careers",
                "href": "https://example.com/careers"
              },
              {
                "label": "Blog",
                "href": "https://example.com/blog"
              },
              {
                "label": "Privacy Policy",
                "href": "https://example.com/legal/privacy"
              }
            ]
          }
        ]
      },
      "integrations": {
        "ga4": {
          "measurementId": "G-XXXXXXXXXX"
        },
        "koala": {
          "publicApiKey": "pk_example_key_123"
        },
        "telemetry": {
          "enabled": true
        },
        "cookies": {
          "key": "example_cookie_key",
          "value": "example_cookie_value"
        }
      },
      "contextual": {
        "options": [
          "copy",
          "view",
          "chatgpt",
          "claude"
        ]
      },
      "errors": {
        "404": {
          "redirect": false,
          "title": "見つかりませんでした",
          "description": "この _ページ_ に一体何が**起きた**のでしょうか？"
        }
      }
    }
    ```
  </Tab>

  <Tab title="Multi-language example">
    ```json title="docs.json" wrap lines
    {
      "$schema": "https://mintlify.com/docs.json",
      "theme": "maple",
      "name": "Example Co.",
      "description": "Example Co. は、サンプル用のコンテンツとプレースホルダー文字列を提供する企業です。",
      "colors": {
        "primary": "#3B82F6",
        "light": "#F8FAFC",
        "dark": "#0F172A"
      },
      "navigation": {
        "global": {
          "anchors": [
            {
              "anchor": "ドキュメント",
              "href": "https://mintlify.com/docs"
            },
            {
              "anchor": "変更履歴",
              "href": "https://mintlify.com/docs/changelog"
            }
          ]
        },
        "languages": [ // [!code highlight:3]
          {
            "language": "en",
            "dropdowns": [
              {
                "dropdown": "ドキュメント",
                "icon": "book",
                "description": "Example Co. 製品の使い方",
                "pages": [
                  {
                    "group": "はじめに",
                    "pages": ["index", "quickstart"]
                  },
                  {
                    "group": "カスタマイズ",
                    "pages": ["settings", "users", "features"]
                  },
                  {
                    "group": "請求",
                    "pages": [
                      "billing/overview",
                      "billing/payments",
                      "billing/subscriptions"
                    ]
                  }
                ]
              },
              {
                "dropdown": "変更履歴",
                "icon": "history",
                "description": "更新情報",
                "pages": ["changelog"]
              }
            ]
          },
          {
            "language": "es",// [!code highlight]
            "dropdowns": [
              {
                "dropdown": "Documentación",
                "icon": "book",
                "description": "Cómo usar el producto de Example Co.",
                "pages": [
                  {
                    "group": "Comenzando",
                    "pages": ["es/index", "es/quickstart"]
                  },
                  {
                    "group": "Personalización",
                    "pages": ["es/settings", "es/users", "es/features"]
                  },
                  {
                    "group": "Billing",
                    "pages": [
                      "es/billing/overview",
                      "es/billing/payments",
                      "es/billing/subscriptions"
                    ]
                  }
                ]
              },
              {
                "dropdown": "Changelog",
                "icon": "history",
                "description": "Actualizaciones y cambios",
                "pages": ["es/changelog"]
              }
            ]
          }
        ]
      },
      "logo": {
        "light": "/logo-light.svg",
        "dark": "/logo-dark.svg",
        "href": "https://example.com"
      },
      "navbar": {
        "links": [
          {
            "label": "コミュニティ",
            "href": "https://example.com/community"
          }
        ],
        "primary": {
          "type": "button",
          "label": "はじめる",
          "href": "https://example.com/start"
        }
      },
      "footer": {
        "socials": {
          "x": "https://x.com/example",
          "linkedin": "https://www.linkedin.com/company/example",
          "github": "https://github.com/example",
          "slack": "https://example.com/community"
        },
        "links": [
          {
            "header": "リソース",
            "items": [
              {
                "label": "導入事例",
                "href": "https://example.com/customers"
              },
              {
                "label": "エンタープライズ",
                "href": "https://example.com/enterprise"
              },
              {
                "label": "プレビューをリクエスト",
                "href": "https://example.com/preview"
              }
            ]
          },
          {
            "header": "会社情報",
            "items": [
              {
                "label": "採用情報",
                "href": "https://example.com/careers"
              },
              {
                "label": "ブログ",
                "href": "https://example.com/blog"
              },
              {
                "label": "プライバシーポリシー",
                "href": "https://example.com/legal/privacy"
              }
            ]
          }
        ]
      },
      "integrations": {
        "ga4": {
          "measurementId": "G-XXXXXXXXXX"
        },
        "koala": {
          "publicApiKey": "pk_example_key_123"
        },
        "telemetry": {
          "enabled": true
        },
        "cookies": {
          "key": "example_cookie_key",
          "value": "example_cookie_value"
        }
      },
      "contextual": {
        "options": ["コピー", "表示", "ChatGPT", "Claude"]
      },
      "errors": {
        "404": {
          "redirect": false,
          "title": "見つかりません",
          "description": "この _ページ_ にいったい何が**起きた**のでしょうか？"
        }
      }
    }
    ```
  </Tab>
</Tabs>

<div id="upgrading-from-mintjson">
  ## `mint.json` からのアップグレード
</div>

ドキュメントプロジェクトで非推奨の `mint.json` を使用している場合は、次の手順に従って `docs.json` にアップグレードしてください。

<Steps>
  <Step title="CLI をインストールまたは更新する">
    まだ [CLI](/ja/installation) をインストールしていない場合は、インストールしてください:

    <CodeGroup>
      ```bash npm
      npm i -g mint
      ```

      ```bash yarn
      yarn global add mint
      ```

      ```bash pnpm
      pnpm add -g mint
      ```
    </CodeGroup>

    すでに CLI をインストール済みの場合は、最新バージョンであることを確認してください:

    ```bash
    mint update
    ```
  </Step>

  <Step title="docs.json ファイルを作成する">
    ドキュメントのリポジトリで次を実行します:

    ```bash
    mint upgrade
    ```

    このコマンドは、既存の `mint.json` から `docs.json` を生成します。生成されたファイルを見直し、すべての設定が正しいことを確認してください。
  </Step>

  <Step title="mint.json ファイルを削除する">
    `docs.json` が正しく構成されていることを確認したら、古い `mint.json` ファイルは安全に削除できます。
  </Step>
</Steps>