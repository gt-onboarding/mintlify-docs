...
[React 组件](https://react.dev) 是在文档中创建交互式、可复用元素的有力工具。
...
此示例先声明一个 `Counter` 组件，然后使用 `<Counter />` 引用它。
...
要在你的 `MDX` 文件中导入 React 组件，组件文件必须位于 `snippets` 文件夹中。随后你可以在文档中的任意 `MDX` 页面中导入它们。进一步了解[可复用片段](/zh/create/reusable-snippets)。

<div id="example">
  ### 示例
</div>

本示例声明了一个使用多个 React 钩子的 `ColorGenerator` 组件，并在一个 `MDX` 文件中使用它。
...
<AccordionGroup>
  <Accordion title="客户端渲染的影响">
    使用 React Hook 的组件在客户端渲染，这会带来一些影响：

    * **SEO**：搜索引擎可能无法完整索引动态内容。
    * **首次加载**：在组件完成渲染前，访客可能会看到短暂的加载占位或闪烁。
    * **无障碍**：确保动态内容的变化能被屏幕阅读器正确宣布。
  </Accordion>
