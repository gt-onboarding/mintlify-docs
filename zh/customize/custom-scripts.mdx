...
你可以使用 CSS 为 HTML 元素设定样式，或添加自定义 CSS 和 JavaScript，全面定制文档的外观与交互体验。
...
将 CSS 文件添加到你的仓库后，其中定义的类名会自动生效，并可在所有 `MDX` 文件中使用。

<div id="adding-stylecss">
  ### 添加 `style.css`
</div>

例如，你可以添加以下 `style.css` 文件，用于自定义导航栏和页脚的样式。
...
Mintlify 提供了一组通用的标识符和选择器，帮助你标记 UI 中的重要元素。
...
自定义 JS 允许你在全局添加自定义可执行代码，相当于在每个页面中插入一个包含 JS 代码的 `<script>` 标签。

<div id="adding-custom-javascript">
  ### 添加自定义 JavaScript
</div>

文档的内容目录中的任意 `.js` 文件都会被包含到每个文档页面中。比如，你可以添加下面的 `ga.js` 文件，在整个文档中启用 [Google Analytics](https://marketingplatform.google.com/about/analytics)。
