...
description: "从单个 MDX 端点页面迁移到基于 OpenAPI 的自动生成，并保留灵活的导航"
icon: "arrow-big-right-dash"
---

如果你目前为 API 端点使用独立的 `MDX` 页面，你可以迁移到基于 OpenAPI 规范自动生成页面，同时保留单个页面的自定义能力。这样可以减少需要维护的文件数量，并提升 API 文档的一致性。
...
推荐使用 `mint migrate-mdx` 命令将 MDX 端点页面迁移为自动生成的页面。
...
<Info>
  如果你已在某个端点上定义了 `x-mint`，且同时存在包含该端点内容的 MDX 页面，则该 MDX 内容将覆盖现有的 `x-mint` 设置。

  如果同一端点存在多个内容不同的 MDX 页面，脚本将采用在你的 `docs.json` 中出现最靠后的那个页面的内容。
...
<Steps>
  <Step title="准备你的 OpenAPI 规范。">
    ...
    需要迁移的 MDX 页面必须在 frontmatter 中使用 `openapi:` 引用某个端点。
  ...
  <Step title="安装 Mint CLI">
    按需安装或更新 [Mint CLI](/zh/installation)。
  </Step>
...
<Steps>
  <Step title="准备你的 OpenAPI 规范。">
    ...
    对于需要自定义元数据或内容的端点，在该端点添加 `x-mint` 扩展。详见 [x-mint 扩展](/zh/api-playground/openapi-setup#x-mint-extension)。

    对于希望从文档中排除的端点，在该端点添加 `x-hidden` 扩展。
