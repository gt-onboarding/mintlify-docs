---
title: "自定义脚本"
description: "通过自定义 CSS 和 JS 完全自定义文档"
icon: "code"
---

使用 CSS 为 HTML 元素设定样式，或添加自定义 CSS 和 JavaScript，以全面自定义文档的外观与体验。
...
例如，你可以添加以下 `style.css` 文件来自定义导航栏和页脚的样式。
...
Mintlify 提供了一组通用的标识符和选择器，帮助你为 UI 中的重要元素打上标签。
...
自定义 JS 允许你在全局添加自定义可执行代码。它等同于在每个页面中插入包含 JS 代码的 `<script>` 标签。

<div id="adding-custom-javascript">
  ### 添加自定义 JavaScript
</div>

文档的内容目录中的任意 `.js` 文件都会被包含在每个文档页面中。比如，你可以添加以下 `ga.js` 文件，在整个文档站点中启用 [Google Analytics](https://marketingplatform.google.com/about/analytics)。
