...
- `rounded-xl` - 大圆角
- `block`、`hidden` - 显示控制
- `dark:hidden`、`dark:block` - 暗色模式下的可见性
...
文档内容目录中的任何 `.js` 文件都会被包含进每个文档页面。比如，你可以添加以下 `ga.js` 文件，在整个文档中启用 [Google Analytics](https://marketingplatform.google.com/about/analytics)。
