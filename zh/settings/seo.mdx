...
<Warning>
  在页面元数据中，包含冒号的 meta 标签键需要使用引号包裹。
</Warning>
...
下面是可添加到你的 `docs.json` 中的 Meta 标签的完整列表。这些标签有助于提升站点的 SEO、社交分享效果和浏览器兼容性。
...
Mintlify 会自动生成 `sitemap.xml` 和 `robots.txt` 文件。你可以在文档站点的 URL 末尾添加 `/sitemap.xml` 以查看站点地图。

默认情况下，只会包含 `docs.json` 中列出的页面。若要包含隐藏链接，请在 `docs.json` 中添加 `seo.indexing`：
...
要添加自定义的 `sitemap.xml` 或 `robots.txt` 文件，请在项目根目录创建相应的 `sitemap.xml` 或 `robots.txt` 文件。添加自定义文件将覆盖同名的自动生成文件。删除自定义文件后，将恢复使用默认文件。

<div id="disabling-indexing">
  ## 禁用收录
</div>

如果你想阻止页面被搜索引擎收录，可以在页面的[前置元数据](zh/pages)中加入以下内容：
