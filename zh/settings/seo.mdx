...
Mintlify 会自动生成 `sitemap.xml` 和 `robots.txt` 文件。你可以在文档站点的 URL 后加上 `/sitemap.xml` 来查看站点地图。

默认情况下，只会收录 `docs.json` 中包含的页面。若要包含隐藏链接，请在 `docs.json` 中添加 `seo.indexing`：
...
如果你想阻止某个页面被搜索引擎收录，可以在页面的[frontmatter](zh/pages)中加入以下内容：
...
<AccordionGroup>
  <Accordion title="撰写清晰有效的标题与描述">
    * 使用清晰、易懂的页面标题（50–60 个字符）
