...
使用字段来编写 API 的参数与响应。字段分为两类：参数字段和响应字段。

<div id="parameter-field">
  ## 参数字段
</div>

`<ParamField>` 组件用于为您的 API 或 SDK 定义参数。添加一个 `ParamField` 会自动添加一个 [API 操作台](/zh/api-playground/overview)。
...
<ParamField body="query, path, body, or header" type="string">
  指明该参数属于 query、path、body 或 header，后接
  参数名称。
</ParamField>
