...
`<ParamField>` 组件用于为你的 API 或 SDK 定义参数。添加 `ParamField` 后会自动生成一个 [API 操作台](/zh/api-playground/overview)。
...
<ParamField body="query, path, body, or header" type="string">
  指明该参数是 query、path、body 还是 header。随后为
  参数名称。
</ParamField>
