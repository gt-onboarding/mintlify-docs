...
可参考 [Mintlify 更新日志](/zh/changelog) 作为示例：每次更新都可以包含链接、图片、文本以及新功能演示。
...
每个 `Update` 的 `label` 属性都会在右侧边栏的目录中自动创建一个条目。这是你的更新日志的默认导航方式。
...
<Frame>
  <img src="/images/changelog-filters-light.png" alt="浅色模式下的更新日志，已选择 Peppermint 标签筛选。" className="block dark:hidden" />

  <img src="/images/changelog-filters-dark.png" alt="深色模式下的更新日志，已选择 Peppermint 标签筛选。" className="hidden dark:block" />
</Frame>

<Tip>
  使用 `custom`、`center` 或 `wide` 页面模式时，目录和更新日志筛选器将被隐藏。了解更多[页面模式](/zh/pages#page-mode)。
</Tip>
...
当发布新的 `Update` 组件，或在已有的 `<Update>` 标签中新增标题时，RSS 源会自动加入对应条目。

`Update` 组件的一级标题会作为 RSS 条目的标题，且这些条目会链接到文档中相应标题的锚点。
...
RSS 源可与 Slack、电子邮件或其他订阅工具集成，用于通知用户产品变更。常见选项包括：
...
* Discord 机器人，如 [Readybot](https://readybot.io) 或 [RSS Feeds to Discord Bot](https://rss.app/en/bots/rssfeeds-discord-bot)

为便于发现 RSS 源，你可以在页面顶部显示一个链接至该源的 RSS 图标按钮。在页面的 frontmatter 中添加 `rss: true`：
...
<Frame>
  <img src="/images/changelog-rss-button-light.png" alt="浅色模式下启用 RSS 源按钮的更新日志页面。" className="block dark:hidden" />

  <img src="/images/changelog-rss-button-dark.png" alt="深色模式下启用 RSS 源按钮的更新日志页面。" className="hidden dark:block" />
</Frame>