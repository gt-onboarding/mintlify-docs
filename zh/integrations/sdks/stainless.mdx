---
title: Stainless
description: 在 API 操作台中自动生成 SDK 示例代码片段
---

<div id="prerequisites">
  ## 先决条件
</div>

* 拥有一个 [Stainless](https://app.stainless.com) 账户。

<div id="integrate-with-stainless">
  ## 集成 Stainless
</div>

<Steps>
  <Step title="在 Stainless 中配置 OpenAPI 装饰。">
    在你的 `stainless.yml` 配置文件中，添加 `openapi.code_samples: 'mintlify'`。更多信息请参阅 [Stainless 文档](https://app.stainless.com/docs/guides/integrate-docs)。
  </Step>

  <Step title="发布你的 OpenAPI 规范 URL。">
    在你的 Stainless 项目中：

    1. 选择 **Release** 选项卡。
    2. 选择 **Setup OpenAPI publishing**。
    3. 复制你公开可访问的 OpenAPI 规范的 URL。

    <img src="/images/stainless-public-OpenAPI-spec.png" alt="Stainless 发布页面，其中 OpenAPI 规范 URL 以绿色框标示。" />
  </Step>

  <Step title={<>将你的 OpenAPI 规范 URL 添加到 <code>docs.json</code>。</>}>
    在你的 `docs.json` 文件中，将来自 Stainless 的该 URL 添加到 `openapi` 字段。更多信息请参阅 [OpenAPI 设置](/zh/api-playground/openapi-setup)。
  </Step>
</Steps>