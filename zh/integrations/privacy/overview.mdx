...
你可以在文档中集成数据隐私平台。在 `docs.json` 文件中添加 `integrations` 字段，并填入对应的脚本。
...
如果你需要为满足 GDPR 合规而检查用户是否已同意使用 Cookie，可以在 `cookies` 下指定本地存储的键和值：
...
如果设置了这些值，系统会检查本地存储以确定用户是否已同意使用 Cookie。若未同意，将禁用遥测。

如果你希望为所有用户禁用遥测，可以在你的 `docs.json` 文件中添加以下内容：
