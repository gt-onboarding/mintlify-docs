...
你可以在文档站点中集成数据隐私平台。将带有相应脚本的 `integrations` 字段添加到你的 `docs.json` 文件中。
...
如果你需要为 GDPR 合规检查用户是否已同意使用 Cookie，可以在 `cookies` 下指定本地存储的键和值：
...
设置这些值后，系统会检查本地存储以确定用户是否已同意使用 Cookie。若未同意，将禁用遥测。

如果你想为所有用户禁用遥测，可以在 `docs.json` 文件中添加以下内容：
