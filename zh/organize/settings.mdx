...
`docs.json` 文件可将一组 Markdown 文件构建为可导航、可自定义的文档站点。这个必需的配置文件控制样式、导航、集成等。可以把它看作你的文档蓝图。
...
开始之前，你只需指定 `theme`、`name`、`colors.primary` 和 `navigation`。其他字段是可选的，你可以在文档需求增长时再逐步添加。

为获得最佳编辑体验，请在 `docs.json` 文件顶部包含架构引用（schema reference）。这将在大多数代码编辑器中启用自动补全、校验和实用的悬浮提示：
...
<ResponseField name="theme" required>
  ...
  参见[主题](/zh/customize/themes)了解更多信息。
</ResponseField>
...
<ResponseField name="colors" type="object" required>
  文档中使用的配色。不同主题的应用方式不同。若仅提供主色，将用于所有颜色元素。
...
<ResponseField name="logo" type="string or object">
  ...
  <Expandable title="Logo">
    ...
    <ResponseField name="href" type="string (uri)">
      点击徽标时跳转的 URL。若未提供，徽标将链接到你的主页。示例：`https://mintlify.com`
    </ResponseField>
  </Expandable>
</ResponseField>

<ResponseField name="favicon" type="string or object">
  指向 favicon 文件的路径，需包含文件扩展名。将自动调整为合适的 favicon 尺寸。可为单个文件，或分别为浅色和深色模式提供。示例：`/favicon.png`
...
<ResponseField name="styling" type="object">
  ...
  <Expandable title="Styling">
    <ResponseField name="eyebrows" type="&#x22;section&#x22; | &#x22;breadcrumbs&#x22;">
      页面 eyebrow 的样式。选择 `section` 显示所属章节名，或选择 `breadcrumbs` 显示完整导航路径。默认为 `section`。
    </ResponseField>
    ...
    <ResponseField name="codeblocks" type="&#x22;system&#x22; | &#x22;dark&#x22; | string | object">
      ...
      * 使用对象分别为亮色和暗色模式指定 [Shiki 主题](https://shiki.style/themes)
...
<ResponseField name="icons" type="object">
  ...
  <Expandable title="Icons">
    <ResponseField name="library" type="&#x22;fontawesome&#x22; | &#x22;lucide&#x22;" required>
      全站文档所用的图标库。默认为 `fontawesome`。

      <Note>
        无论库设置为何，您都可以为任意单个图标指定外部托管图标的 URL、项目内图标文件的路径，或可用于 JSX 的 SVG 代码。
      </Note>
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="fonts" type="object">
  ...
  <Expandable title="Fonts">
    <ResponseField name="family" type="string" required>
      字体系列，例如 “Open Sans”。支持使用 [Google Fonts](https://fonts.google.com) 的 family 名称。
    </ResponseField>
    ...
    <ResponseField name="source" type="string (uri)">
      ...
      * 本地字体文件的路径，例如 `/fonts/Hubot-Sans.woff2`。
      ...
      当你指定 Google Font 的 `family` 名称时，[Google Fonts](https://fonts.google.com) 会自动加载，因此无需提供 source URL。
    </ResponseField>
    ...
    <ResponseField name="heading" type="object">
      ...
      <Expandable title="Heading">
        <ResponseField name="family" type="string" required>
          字体系列，例如 “Open Sans”、“Playfair Display”。支持使用 [Google Fonts](https://fonts.google.com) 的 family 名称。
        </ResponseField>

        <ResponseField name="weight" type="number">
          字重，例如 400、700。可变字体支持更精确的字重，如 550。
        </ResponseField>
...
<ResponseField name="appearance" type="object">
  ...
  <Expandable title="Appearance">
    <ResponseField name="default" type="&#x22;system&#x22; | &#x22;light&#x22; | &#x22;dark&#x22;">
      默认主题模式。选择 `system` 以匹配用户的操作系统设置，或选择 `light` 或 `dark` 以强制指定模式。默认值为 `system`。
    </ResponseField>
...
<ResponseField name="background" type="object">
  ...
  <Expandable title="Background">
    <ResponseField name="image" type="string or object">
      站点的背景图片。可以是单个文件，或分别为浅色和深色模式提供文件。
    ...
    <ResponseField name="primary" type="object">
      ...
      <Expandable title="Primary">
        <ResponseField name="type" type="&#x22;button&#x22; | &#x22;github&#x22;" required>
          按钮类型。选择 `button` 以使用带文本的标准按钮，或选择 `github` 以使用带图标的 GitHub 仓库链接。
        </ResponseField>
        ...
        <ResponseField name="href" type="string (uri)" required>
          按钮链接。必须为外部 URL；若 `type` 为 `github`，则必须是 GitHub 仓库的 URL。
        </ResponseField>
      </Expandable>
    </ResponseField>
  </Expandable>
...
<ResponseField name="导航" type="object" required>
  ...
  <Expandable title="导航">
    ...
    <ResponseField name="语言" type="array of object">
      适用于[多语言](organize/navigation#languages)站点的语言切换器。
    </ResponseField>

    <ResponseField name="版本" type="array of object">
      用于拥有多个[版本](organize/navigation#versions)的网站的版本切换器。
    </ResponseField>

    <ResponseField name="标签页" type="array of object">
      顶级导航[选项卡](organize/navigation#tabs)。
    </ResponseField>
    ...
    <ResponseField name="下拉菜单" type="array of object">
      [下拉菜单](organize/navigation#dropdowns) 用于分组相关内容。
    </ResponseField>

    <ResponseField name="产品" type="array of object">
      适用于拥有多个[产品](organize/navigation#products)的网站的“产品切换器”。
    </ResponseField>

    <ResponseField name="组" type="array of object">
      用于将内容组织成章节的[Groups](organize/navigation#groups)。
    </ResponseField>

    <ResponseField name="页面" type="array of string or object">
      构成文档的单个[页面](organize/navigation#pages)。
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="interaction" type="object">
  ...
  <Expandable title="Interaction">
    <ResponseField name="drilldown" type="boolean">
      控制选择导航分组时的自动导航行为。设为 `true` 时，在展开导航分组时会强制跳转到该分组的第一页；设为 `false` 时，仅展开或收起分组而不跳转。不设置则采用主题的默认行为。
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="footer" type="object">
  ...
  <Expandable title="Footer">
    <ResponseField name="socials" type="object">
      在页脚中显示的社交媒体账号。每个键为平台名称，每个值为你的个人资料 URL。例如：
    ...
    <ResponseField name="examples" type="object">
      ...
      <Expandable title="Examples">
        ...
        <ResponseField name="prefill" type="boolean">
          是否使用架构示例数据预填充 API 操作台。启用后，操作台会自动将 OpenAPI 规范中的示例值填入请求字段。默认值为 `false`。
        </ResponseField>
      </Expandable>
    </ResponseField>
...
<ResponseField name="seo" type="object">
  ...
  <Expandable title="Seo">
    <ResponseField name="metatags" type="object">
      添加到每个页面的 Meta 标签。必须是有效的键值对。选项参见[常用 Meta 标签参考](/zh/optimize/seo#common-meta-tags-reference)。
    </ResponseField>

    <ResponseField name="indexing" type="&#x22;navigable&#x22; | &#x22;all&#x22;">
      指定搜索引擎应索引的页面。选择 `navigable` 仅索引 `docs.json` 导航中的页面，选择 `all` 则索引所有页面。默认值为 `navigable`。
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="集成" type="object">
  ...
  <Expandable title="集成">
    ...
    <ResponseField name="英寻" type="object">
      Fathom 分析集成。
    ...
    <ResponseField name="可行的" type="object">
      Plausible 分析集成。
    ...
    <ResponseField name="分段" type="object">
      Segment 集成。
...
如果你的文档项目仍在使用已弃用的 `mint.json` 文件，请按照以下步骤升级到 `docs.json`。
...
<Steps>
  ...
  <Step title="删除 mint.json 文件">
    在确认 `docs.json` 配置无误后，你可以安心删除旧的 `mint.json` 文件。
  </Step>
</Steps>