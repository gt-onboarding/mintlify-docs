...
title: "内容安全策略（CSP）配置"
sidebarTitle: "CSP 配置"
description: "适用于实施严格安全策略的反向代理、防火墙和网络的域名允许列表与响应头配置"
---

内容安全策略（CSP）是一项安全标准，通过控制网页可加载的资源来帮助防止跨站脚本（XSS）攻击。Mintlify 提供了适用于大多数站点的默认 CSP。若你的文档托管在会覆盖默认 CSP 的反向代理或防火墙之后，你可能需要配置 CSP 响应头，才能确保各项功能正常运行。
...
| `cdn.jsdelivr.net` | OG 图片的 Emoji 资源 | `script-src`、`img-src` | 必需 |
