...
[I componenti React](https://react.dev) sono un modo potente per creare elementi interattivi e riutilizzabili nella documentazione.
...
Questo esempio dichiara un componente `Counter` e quindi lo utilizza con `<Counter />`.
...
Il contatore viene renderizzato come un componente React interattivo.
...
Puoi importare componenti dalla cartella `snippets`. A differenza di React, non puoi importare componenti da qualunque file `MDX`. I componenti riutilizzabili devono essere richiamati da file all’interno della cartella `snippets`. Scopri di più sugli [snippet riutilizzabili](/it/reusable-snippets).

<div id="example">
  ### Esempio
</div>

Questo esempio dichiara un componente `ColorGenerator` che usa più hook di React e poi lo utilizza in un file `MDX`.
...
Importa il componente `ColorGenerator` e usalo in un file `MDX`:
...
<AccordionGroup>
  ...
  <Accordion title="Best practice per le prestazioni">
    ...
    * **Memoizza i calcoli complessi**: Usa `useMemo` o `useCallback` per operazioni costose.
    * **Riduci i re-render**: Suddividi i componenti grandi in parti più piccole per evitare re-render a cascata.
