...
Vedi la sezione [Formato dei dati utente](#user-data-format) qui sotto per esempi dettagliati e indicazioni di implementazione.

<div id="page-visibility">
  ### Visibilità delle pagine
</div>

Limita quali pagine sono visibili ai tuoi utenti aggiungendo il campo `groups` al frontmatter delle pagine. Per impostazione predefinita, ogni pagina è visibile a qualsiasi utente.
...
>
  ...
  <Warning><b>Per gli handshake JWT:</b> Questo campo è diverso dalla claim `exp` del JWT, che determina quando un JWT è considerato non valido. Imposta la claim `exp` del JWT su una durata breve (10 secondi o meno) per motivi di sicurezza. Usa `expiresAt` per la durata effettiva della sessione (da ore a settimane).</Warning>
</ParamField>
...
<ParamField
  ...
  type="string[]"
>
  Elenco dei gruppi a cui appartiene l’utente. Le pagine con `groups` corrispondenti nel frontmatter sono visibili a questo utente.

  **Esempio**: Un utente con `groups: ["admin", "engineering"]` può accedere alle pagine etichettate con il gruppo `admin` o `engineering`.
</ParamField>
...
>
  ...
  **Utilizzo in `MDX`**:
  ...
  ```
  Con i dati di `user` dell’esempio, il risultato sarà: Bentornato, Ronan! Il tuo piano Enterprise include...
...
<ParamField
  ...
  type="object"
>
  Valori specifici dell’utente che precompilano i campi dell’API playground. Fanno risparmiare tempo popolando automaticamente i dati dell’utente durante il test delle API.
  ...
  **Esempio**:
  ...
  <Note>I campi `header`, `query` e `cookie` verranno precompilati solo se fanno parte dello [schema di sicurezza OpenAPI](https://swagger.io/docs/specification/authentication/). Se un campo è nelle sezioni `Authorization` o `Server`, verrà precompilato. Creare un parametro header standard chiamato `Authorization` non abilita questa funzionalità.</Note>
</ParamField>
...
<Tabs>
  <Tab title="JWT">
    ...
    * Un servizio backend in grado di creare URL di reindirizzamento
    ...
    <Steps>
      ...
      <Step title="Integra la personalizzazione di Mintlify nel tuo flusso di login.">
        Modifica il flusso di login esistente per includere questi passaggi dopo l’accesso dell’utente:

        * Crea un JWT contenente le informazioni dell'utente autenticato nel formato `User`. Consulta la sezione [User data format](#user-data-format) per maggiori informazioni.
    ...
    La tua documentazione è ospitata su `docs.foo.com`. Vuoi tenerla separata dal tuo dashboard (o non hai un dashboard) e abilitare la personalizzazione.

    Genera un secret per i JWT. Quindi crea un endpoint di login su `https://foo.com/docs-login` che avvii un flusso di login verso la documentazione.
    ...
    * Possibilità di creare un endpoint API accessibile tramite token di accesso OAuth
    ...
    <Steps>
      <Step title="Create user info API endpoint.">
        ...
        * Restituisca i dati utente nel formato `User`. Vedi la sezione [Formato dei dati utente](#user-data-format) sopra per ulteriori informazioni.
      ...
      <Step title="Configure your OAuth personalization settings.">
        ...
        * **Client ID**: L’identificatore client OAuth 2.0.
        * **Scopes**: Permessi da richiedere. Copia l’**intera** stringa dello scope (ad esempio, per uno scope come `provider.users.docs`, copia per intero `provider.users.docs`). Deve corrispondere agli scope dell’endpoint configurato nel primo passaggio.
    ...
    La tua documentazione è ospitata su `foo.com/docs` e disponi di un server OAuth esistente che supporta il flusso PKCE. Vuoi personalizzare la documentazione in base ai dati utente.

    **Crea un endpoint di informazioni utente** in `api.foo.com/docs/user-info`, che richiede un token di accesso OAuth con lo scope `provider.users.docs` e risponde con i dati personalizzati dell’utente:
  ...
  <Tab title="Shared session">
    ...
    * Possibilità di creare un endpoint API sulla stessa origin o su un sottodominio della tua dashboard.
    ...
    * La tua documentazione è ospitata sullo stesso dominio o su un sottodominio della tua dashboard.
    ...
    <Steps>
      <Step title="Create user info API endpoint.">
        ...
        * Se il dominio dell'API e quello della documentazione **non coincidono esattamente**:
      ...
      <Step title="Configure your personalization settings">
        ...
        4. Inserisci il tuo **Info API URL**, ovvero l'endpoint del primo passaggio.
        5. Inserisci il tuo **Login URL**, dove gli utenti eseguono l'accesso alla dashboard.
    ...
    Hai una dashboard su `dash.foo.com`, che usa l'autenticazione di sessione basata su cookie. Le route API della tua dashboard sono ospitate su `dash.foo.com/api`. Vuoi configurare la personalizzazione per la tua documentazione ospitata su `docs.foo.com`.
    ...
    **Procedura di configurazione**:
    ...
    Hai una dashboard su `dash.foo.com`, che usa l'autenticazione di sessione basata su cookie. Le route API della tua dashboard sono ospitate su `dash.foo.com/api`. Vuoi configurare la personalizzazione per la tua documentazione ospitata su `foo.com/docs`.
    ...
    **Procedura di configurazione**:
    ...
    Hai una dashboard su `foo.com/dashboard`, che usa l'autenticazione di sessione basata su cookie. Le route API della tua dashboard sono ospitate su `foo.com/api`. Vuoi configurare la personalizzazione per la tua documentazione ospitata su `foo.com/docs`.
