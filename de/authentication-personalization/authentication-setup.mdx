...
description: "Steuern Sie die Zugriffsrechte Ihrer Dokumentation durch Benutzeranmeldung"
icon: "file-lock"
keywords: ['auth']
---
...
**Partielle Authentifizierung**: Einige Seiten sind öffentlich zugänglich, während andere eine Anmeldung erfordern. Nutzer können öffentliche Inhalte frei durchsuchen und müssen sich nur beim Zugriff auf geschützte Seiten authentifizieren.

Wenn Sie eine der unten genannten Handshake-Methoden konfigurieren, wählen Sie in den Dashboard-Einstellungen entweder **Vollständige Authentifizierung** oder **Partielle Authentifizierung**.
...
<Tabs>
  <Tab title="Password">
    ...
    * Ihre Sicherheitsanforderungen erlauben das gemeinsame Nutzen von Passwörtern durch Benutzer.
    ...
    Ihre Dokumentation wird unter `docs.foo.com` gehostet und Sie benötigen eine grundlegende Zugriffskontrolle, ohne einzelne Benutzer nachzuverfolgen. Sie möchten den öffentlichen Zugriff verhindern und die Einrichtung dennoch einfach halten.

    **Erstellen Sie ein starkes Passwort** in Ihrem Dashboard. **Teilen Sie die Zugangsdaten** mit autorisierten Benutzern. Das war’s!
  </Tab>

  <Tab title="Mintlify Dashboard">
    ### Voraussetzungen

    * Ihre Dokumentationsnutzer sind auch Ihre Dokumentationseditoren.
    ...
    Ihre Dokumentation ist unter `docs.foo.com` gehostet und Ihr Team nutzt das Dashboard, um die Docs zu bearbeiten. Sie möchten den Zugriff ausschließlich auf Teammitglieder beschränken.
  ...
  <Tab title="OAuth 2.0">
    ...
    * Möglichkeit, einen API-Endpunkt zu erstellen, der über OAuth-Zugriffstokens erreichbar ist (optional, um Personalisierungsfunktionen zu aktivieren).
    ...
    <Steps>
      <Step title="Konfigurieren Sie Ihre OAuth-Einstellungen.">
        ...
        * **Scopes**: Anzufordernde Berechtigungen. Kopieren Sie die **gesamte** Scope-Zeichenfolge (zum Beispiel bei einem Scope wie `provider.users.docs` das vollständige `provider.users.docs`). Verwenden Sie mehrere Scopes, wenn Sie unterschiedliche Zugriffsebenen benötigen.
        * **Token URL**: Ihr OAuth-Endpunkt für den Token-Austausch.
        * **Info API URL** (optional): Endpunkt zum Abrufen von Benutzerdaten für die Personalisierung. Wenn weggelassen, wird der OAuth-Flow nur zur Identitätsprüfung verwendet und die Benutzerinformationen bleiben leer.
        * **Logout URL**: Die native Logout-URL Ihres OAuth-Providers. Wenn Ihr Provider einen Parameter wie `returnTo` anbietet, verweisen Sie zurück auf Ihre Docs-URL.
    ...
    Ihre Dokumentation ist unter `foo.com/docs` gehostet, und Sie haben einen vorhandenen OAuth-Server unter `auth.foo.com`, der den Authorization Code Flow unterstützt.
  ...
  <Tab title="JWT">
    ...
    * Ein Backend-Service, der Redirect-URLs erstellen kann.
    ...
    <Steps>
      ...
      <Step title="Mintlify-Authentifizierung in Ihren Login-Flow integrieren.">
        Passen Sie Ihren bestehenden Login-Flow an, um nach der Benutzeranmeldung diese Schritte auszuführen:
        ...
        * Erstellen Sie eine Redirect-URL zurück zum Pfad `/login/jwt-callback` Ihrer Docs und fügen Sie das JWT als Hash an.
      </Step>
    </Steps>

    ### Beispiel

    Ihre Dokumentation ist unter `docs.foo.com` gehostet, Ihr bestehendes Authentifizierungssystem unter `foo.com`. Sie möchten Ihren Login-Flow erweitern, um Zugriff auf die Docs zu gewähren, während die Docs weiterhin vom Dashboard getrennt bleiben (oder Sie haben kein Dashboard).
    ...
    * Signieren Sie das JWT und leiten Sie weiter zu `https://docs.foo.com/login/jwt-callback#{SIGNED_JWT}`.
    ...
    <CodeGroup>
      ...
      export async function handleRequest(req: Request, res: Response) {
        ...
        const jwt = await new jose.SignJWT(user)
          ...
          .setExpirationTime('10 s') // JWT läuft nach 10 Sekunden ab
          .sign(signingKey);
      ...
      async def return_mintlify_auth_status(current_user):
        jwt_token = jwt.encode(
          payload={
            'exp': int((datetime.now() + timedelta(seconds=10)).timestamp()),    # JWT läuft nach 10 Sekunden ab
    ...
    Wenn ein nicht authentifizierter Benutzer versucht, auf eine geschützte Seite zuzugreifen, bleibt sein ursprüngliches Ziel in der Weiterleitung zu Ihrer Login-URL erhalten:
...
```json Öffentliches Gruppenbeispiel
{
  "navigation": {
    "groups": [
      {
