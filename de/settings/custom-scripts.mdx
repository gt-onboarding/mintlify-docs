...
Beliebige Werte in Tailwind CSS werden nicht unterstützt. Für benutzerdefinierte Werte verwende stattdessen das `style`-Prop.
...
Sie können zum Beispiel die folgende Datei `style.css` hinzufügen, um die Navigation und die Fußzeile zu gestalten.
...
<div id="using-identifiers-and-selectors">
  ### Verwendung von Bezeichnern und Selektoren
</div>

Mintlify stellt eine Reihe gängiger Bezeichner und Selektoren bereit, mit denen Sie wichtige Elemente der Benutzeroberfläche markieren können.

<Tip>
  Verwenden Sie die Funktion „Element untersuchen“, um Referenzen auf die Elemente zu finden, die Sie anpassen möchten.
</Tip>
...
<Warning>
  Verweise und das Styling gängiger Elemente können sich ändern, wenn sich die Plattform weiterentwickelt. Bitte verwenden Sie benutzerdefiniertes Styling mit Vorsicht.
</Warning>
...
Jede `.js`-Datei im Inhaltsverzeichnis Ihrer Doku wird auf jeder Dokumentationsseite eingebunden. Sie können beispielsweise die folgende Datei `ga.js` hinzufügen, um [Google Analytics](https://marketingplatform.google.com/about/analytics) in der gesamten Dokumentation zu aktivieren.
