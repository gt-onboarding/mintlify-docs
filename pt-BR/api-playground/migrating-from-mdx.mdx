...
O comando `mint migrate-mdx` é a maneira recomendada de migrar de páginas de endpoint em MDX para páginas geradas automaticamente.
...
<Steps>
  <Step title="Prepare sua especificação OpenAPI.">
    ...
    Para endpoints cujo conteúdo ou metadados você queira personalizar, adicione a extensão `x-mint` ao endpoint. Consulte [extensão x-mint](/pt-BR/api-playground/openapi-setup#x-mint-extension) para mais detalhes.
