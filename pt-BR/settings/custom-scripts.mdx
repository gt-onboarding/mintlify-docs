...
description: "Personalize sua documentação por completo com CSS e JS personalizados"
icon: "code"
---
...
Valores arbitrários do Tailwind CSS não são suportados. Para valores personalizados, use a prop `style`.
...
Por exemplo, você pode adicionar o seguinte arquivo `style.css` para personalizar a aparência da barra de navegação e do rodapé.
...
<Warning>
  As referências e a formatação de elementos comuns estão sujeitas a alterações conforme a plataforma evolui. Use formatações personalizadas com cautela.
</Warning>
...
O JavaScript personalizado permite adicionar código executável de forma global. É equivalente a incluir uma tag `<script>` com código JS em todas as páginas.
