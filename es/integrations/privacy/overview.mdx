...
Puede añadir plataformas de privacidad de datos a su documentación. Añada el campo `integrations` a su archivo `docs.json` con los scripts correspondientes.
...
Si necesitas verificar si un usuario ya ha dado su consentimiento para las cookies a efectos de cumplimiento del RGPD, puedes especificar una clave y un valor de almacenamiento local en `cookies`:
...
Si se configuran estos valores, se consultará el almacenamiento local para comprobar si el usuario ha consentido el uso de cookies. Si no lo ha hecho, se desactivará la telemetría.
