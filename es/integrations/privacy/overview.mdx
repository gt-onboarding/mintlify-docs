...
Puedes añadir plataformas de privacidad de datos a tu documentación. Agrega el campo `integrations` en tu archivo `docs.json` con los scripts correspondientes.
...
Si necesitas comprobar si un usuario ya ha dado su consentimiento a las cookies para cumplir con el RGPD, puedes especificar una clave y un valor de almacenamiento local en `cookies`:
...
Si se definen estos valores, se consultará el almacenamiento local para verificar si el usuario ha aceptado las cookies. Si no las ha aceptado, se desactivará la telemetría.
