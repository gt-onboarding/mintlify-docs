...
Elige entre autenticación completa y autenticación parcial según tus necesidades de control de acceso.
...
**Autenticación parcial**: Algunas páginas son de acceso público mientras que otras requieren autenticación. Los usuarios pueden explorar el contenido público libremente y autenticarse solo al acceder a páginas protegidas.
...
<Tabs>
  <Tab title="Contraseña">
    <Info>
      La autenticación por contraseña solo proporciona control de acceso y **no** admite la Personalización de contenido.
    </Info>
    ...
    * Tus requisitos de seguridad permiten compartir contraseñas entre usuarios.
    ...
    <Steps>
      <Step title="Crea una contraseña.">
        ...
        2. Selecciona **Full Authentication** o **Autenticación parcial**.
  ...
  <Tab title="Panel de control de Mintlify">
    ...
    * Las personas que consultan tu documentación también son quienes la editan.
  ...
  <Tab title="OAuth 2.0">
    ...
    * Capacidad para crear un endpoint de API accesible mediante tokens de acceso de OAuth (opcional, para habilitar la Personalización).

    ### Implementación

    <Steps>
      <Step title="Configure la configuración de OAuth.">
        ...
        * **Scopes**: Permisos a solicitar. Copie la cadena de scope **completa** (por ejemplo, para un scope como `provider.users.docs`, copie el `provider.users.docs` completo). Use varios scopes si necesita distintos niveles de acceso.
        * **Token URL**: Su endpoint de intercambio de tokens de OAuth.
        * **Info API URL** (opcional): Endpoint para obtener información del usuario para la Personalización. Si se omite, el flujo de OAuth solo se usará para verificar la identidad y la información del usuario estará vacía.
        * **Logout URL**: La URL nativa de cierre de sesión de su proveedor de OAuth. Si su proveedor tiene un parámetro `returnTo` o similar, rediríjalo de vuelta a la URL de su documentación.

        4. Seleccione **Save changes**.
      </Step>

      <Step title="Configure su servidor OAuth.">
        1. Copie la **Redirect URL** desde su [authentication settings](https://dashboard.mintlify.com/settings/deployment/authentication).
        2. Agregue la Redirect URL como una URL de redirección autorizada en su servidor OAuth.
      </Step>

      <Step title="Cree su endpoint de información de usuario (opcional).">
        Para habilitar la Personalización, cree un endpoint de API que:

        * Acepte tokens de acceso de OAuth para la autenticación.
        * Devuelva datos de usuario en el formato `User`. Consulte [User data format](/es/deploy/personalization-setup#user-data-format) para obtener más información.

        Agregue la URL de este endpoint al campo **Info API URL** en sus [authentication settings](https://dashboard.mintlify.com/settings/deployment/authentication).
      </Step>
    </Steps>
    ...
    * Un servicio de backend que pueda crear URL de redirección.
    ...
    <Steps>
      ...
      <Step title="Integra la autenticación de Mintlify en tu flujo de inicio de sesión.">
        ...
        * Crea un JWT que contenga la información del usuario autenticado en el formato `User`. Consulta [User data format](/es/deploy/personalization-setup#user-data-format) para obtener más información.
...
Cuando se usa la autenticación parcial, todas las páginas están protegidas de forma predeterminada. Puedes hacer que páginas específicas sean visibles sin autenticación, ya sea a nivel de página o de grupo, usando la propiedad `public`.
