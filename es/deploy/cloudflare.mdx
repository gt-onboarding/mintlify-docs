...
<Info>
  Antes de comenzar, necesitas una cuenta de Cloudflare y un nombre de dominio (puede administrarse dentro o fuera de Cloudflare).
</Info>
...
Los archivos de documentación deben organizarse dentro del repositorio para que coincidan con la subruta que elijas. Por ejemplo, si quieres que tu documentación esté en `yoursite.com/docs`, crea un directorio `docs/` con todos tus archivos de documentación.
...
Si usas Cloudflare como proxy junto con implementaciones de Vercel, debes asegurarte de que la configuración sea adecuada para evitar conflictos con la verificación de dominio de Vercel y la emisión de certificados SSL.
...
<Tip>
  Reemplaza “[SUBDOMAIN]” por tu subdominio único, “[YOUR&#95;DOMAIN]” por la URL base de tu sitio web y “/docs” por la subruta que prefieras si es distinta.
</Tip>
...
async function handleRequest(request) {
  try {
    ...
    // Si la solicitud es a una ruta de verificación de Vercel, permitirla pasar
    ...
    // Si no se encuentra ninguna acción, procesar la solicitud regular
    return await fetch(request);
  }
}
```
...
<Tip>
  Recomendamos agregar tu dominio tanto con `www.` como sin `www.` al inicio.
</Tip>
...
<div id="webflow-custom-routing">
  ## Enrutamiento personalizado con Webflow
</div>

Si usas Webflow para alojar tu sitio principal y quieres servir la documentación de Mintlify en `/docs` en el mismo dominio, deberás configurar un enrutamiento personalizado con Cloudflare Workers para redirigir mediante proxy todo el tráfico que no sea de docs a tu sitio principal.

<Warning>
  Asegúrate de que tu sitio principal esté configurado en una página de destino antes de implementar este Worker; de lo contrario, los visitantes de tu sitio principal verán errores.
</Warning>
...
3. Para evitar conflictos, actualiza las URL absolutas de tu sitio principal para que sean relativas.
4. En Cloudflare, selecciona **Edit Code** y agrega el siguiente script en el código de tu Worker.

<Tip> Reemplaza `[SUBDOMAIN]` por tu subdominio único, `[YOUR_DOMAIN]` por la URL base de tu sitio web, `[LANDING_DOMAIN]` por la URL de tu página de destino y `/docs` por la subruta que quieras usar si es distinta. </Tip>
...
```javascript
  ...
  try {
    ...
    // Si la solicitud es a una ruta de verificación de Vercel, permitir que continúe
    if (urlObject.pathname.startsWith('/.well-known/')) {
      return await fetch(request);
    }
