...
La personalización adapta tu documentación para cada usuario cuando inicia sesión. Por ejemplo, puedes autocompletar sus claves de API, mostrar contenido específico según su plan o rol, u ocultar secciones a las que no necesitan acceder.
...
<div id="api-key-prefilling">
  ### Autorrellenado de claves de API
</div>

Completa automáticamente los campos del Área de pruebas de API con valores específicos del usuario devolviendo nombres de campo que coincidan en tus datos de usuario. Los nombres de campo en tus datos de usuario deben coincidir exactamente con los nombres del Área de pruebas de API para que el autorrellenado funcione.

<div id="dynamic-mdx-content">
  ### Contenido MDX dinámico
</div>

Muestra contenido dinámico en función de información del usuario, como el nombre, el plan u organización, usando la variable `user`.

```jsx
¡Te damos la bienvenida de nuevo, {user.firstName}! Tu plan {user.org?.plan} incluye...
```
...
Restringe qué páginas son visibles para tus usuarios añadiendo campos `groups` al front matter de tus páginas. De forma predeterminada, todas las páginas son visibles para todos los usuarios.

Los usuarios solo verán las páginas de los `groups` a los que pertenecen.

```mdx
---
title: "Administrar tus usuarios"
...
Al implementar la Personalización, tu sistema devuelve datos de usuario en un formato específico que permite personalizar el contenido. Estos datos pueden enviarse como un objeto JSON sin procesar o dentro de un JWT firmado, según tu método de autenticación/handshake. La estructura de los datos es la misma en ambos casos.
...
<ParamField path="expiresAt" type="number">
  Tiempo de expiración de la sesión en **segundos desde la época Unix**. Si el usuario carga una página después de este momento, sus datos almacenados se eliminan automáticamente y deberá volver a autenticarse.
  <Warning><b>Para handshakes JWT:</b> Esto difiere del claim `exp` del JWT, que determina cuándo se considera inválido un JWT. Por seguridad, establezca el claim `exp` del JWT en una duración corta (10 segundos o menos). Use `expiresAt` para la duración real de la sesión (de horas a semanas).</Warning>
</ParamField>

<ParamField path="groups" type="string[]">
  Lista de grupos a los que pertenece el usuario. Las páginas cuyo frontmatter tenga `groups` coincidentes serán visibles para este usuario.
...
<ParamField path="content" type="object">
  Datos personalizados accesibles en su contenido `MDX` mediante la variable `user`. Úselo para Personalización dinámica en toda su documentación.
  ...
  **Uso en `MDX`**:
  ...
  Con los datos de `user` del ejemplo, se mostraría así: Welcome back, Ronan! Your Enterprise plan includes...
  ...
  **Renderizado condicional avanzado**:
  ...
  <Note>
    La información en `user` solo está disponible para usuarios con sesión iniciada. Para usuarios que no hayan iniciado sesión, el valor de `user` será `{}`. Para evitar que la página falle en usuarios sin sesión iniciada, utilice siempre encadenamiento opcional en los campos de `user`. Por ejemplo, `{user.org?.plan}`.
  </Note>
</ParamField>

<ParamField path="apiPlaygroundInputs" type="object">
  Valores específicos del usuario que rellenan previamente los campos del Área de pruebas de API. Ahorra tiempo a los usuarios al autocompletar sus datos al probar APIs.
  ...
  **Ejemplo**:
  ...
  Si un usuario realiza solicitudes en un subdominio específico, puede enviar `{ server: { subdomain: 'foo' } }` como un campo `apiPlaygroundInputs`. Este valor se rellenará previamente en cualquier página de API con el valor `subdomain`.

  <Note>Los campos `header`, `query` y `cookie` solo se rellenarán previamente si forman parte de su [esquema de seguridad de OpenAPI](https://swagger.io/docs/specification/authentication/). Si un campo está en las secciones `Authorization` o `Server`, se rellenará previamente. Crear un parámetro de encabezado estándar llamado `Authorization` no habilitará esta función.</Note>
</ParamField>
...
<Tabs>
    ...
    * Un servicio de backend que pueda crear URL de redirección
    ...
    <Steps>
      <Step title="Genera una clave privada.">
        ...
        6. Almacena tu clave de forma segura donde pueda accederla tu backend.
      </Step>
      ...
      <Step title="Integra la personalización de Mintlify en tu flujo de inicio de sesión.">
        ...
        * Crea un JWT que contenga la información del usuario autenticado en el formato `User`. Consulta la sección [User data format](#user-data-format) anterior para obtener más información.
  ...
  <Tab title="OAuth 2.0">
    ...
    * Capacidad para crear un endpoint de API accesible mediante tokens de acceso de OAuth
    ...
    <Steps>
      <Step title="Crea el endpoint de API de información de usuario.">
      ...
      <Step title="Configura los ajustes de personalización de OAuth.">
        1. En tu panel, ve a [Authentication](https://dashboard.mintlify.com/settings/deployment/authentication).
        ...
        * **Scopes**: Permisos a solicitar. Copia la cadena de scope **completa** (por ejemplo, para un scope como `provider.users.docs`, copia `provider.users.docs` completo). Debe coincidir con los scopes del endpoint que configuraste en el primer paso.
        * **Token URL**: Tu endpoint de intercambio de tokens de OAuth.
        * **Info API URL**: Endpoint para obtener los datos de usuario para la Personalización. Creado en el primer paso.
    ...
    **Configura los detalles de tu servidor OAuth** en tu panel de control:

    * **URL de autorización**: `https://auth.foo.com/authorization`
    * **ID de cliente**: `ydybo4SD8PR73vzWWd6S0ObH`
    * **Alcances (scopes)**: `['docs-user-info']`
  ...
  <Tab title="Sesión compartida">
    ...
    * Tu documentación está alojada en el mismo dominio o subdominio que tu dashboard.
    ...
    <Steps>
      <Step title="Create user info API endpoint.">
        ...
        * Devuelva los datos del usuario en el formato `User` (consulta la sección [User data format](#user-data-format) de arriba)
        * Si el dominio de la API y el de la documentación **no coinciden exactamente**:
          ...
          * Establece el encabezado `Access-Control-Allow-Credentials` de tu API en `true`.
          ...
          <Warning>
            Habilita los encabezados CORS solo en este endpoint específico, no en toda la API de tu dashboard.
          </Warning>
      </Step>
    ...
    #### Dashboard en subdominio, documentación en subdominio

    Tienes un dashboard en `dash.foo.com`, que usa autenticación de sesión basada en cookies. Las rutas de la API de tu dashboard están alojadas en `dash.foo.com/api`. Quieres configurar la personalización para tu documentación alojada en `docs.foo.com`.

    **Proceso de configuración**:

    1. **Crea el endpoint** `dash.foo.com/api/docs/user-info` que identifique a los usuarios mediante autenticación de sesión y responda con sus datos.
    ...
    #### Dashboard en subdominio, documentación en raíz

    Tienes un dashboard en `dash.foo.com`, que usa autenticación de sesión basada en cookies. Las rutas de la API de tu dashboard están alojadas en `dash.foo.com/api`. Quieres configurar la personalización para tu documentación alojada en `foo.com/docs`.
    ...
    **Proceso de configuración**:
    ...
    Tienes un dashboard en `foo.com/dashboard`, que usa autenticación de sesión basada en cookies. Las rutas de la API de tu dashboard están alojadas en `foo.com/api`. Quieres configurar la personalización para tu documentación alojada en `foo.com/docs`.

    **Proceso de configuración**:

    1. **Crea el endpoint** `foo.com/api/docs/user-info` que identifique a los usuarios mediante autenticación de sesión y responda con sus datos.
    2. **Configura la URL de la API** en la configuración de autenticación: `https://foo.com/api/docs/user-info`

    <Note>
      No se necesita configuración de CORS, ya que el dashboard y la documentación comparten el mismo dominio.
    </Note>
  </Tab>
</Tabs>