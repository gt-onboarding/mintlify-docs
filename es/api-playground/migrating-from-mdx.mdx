...
Si actualmente usas páginas individuales en `MDX` para tus endpoints de API, puedes migrar a generar páginas automáticamente desde tu especificación de OpenAPI manteniendo la capacidad de personalizar cada página. Esto puede ayudarte a reducir la cantidad de archivos que debes mantener y mejorar la coherencia de tu documentación de API.

Puedes definir metadatos y contenido para cada endpoint en tu especificación de OpenAPI y ubicar los endpoints donde quieras en la navegación.
...
<Info>
  ...
  Si tienes varias páginas MDX para el mismo endpoint con contenido diferente, el script usará el contenido de la página que aparezca última en tu `docs.json`.
...
<Steps>
  <Step title="Prepara tu especificación de OpenAPI.">
    ...
    Para cualquier endpoint cuyo contenido o metadatos quieras personalizar, agrega la extensión `x-mint` al endpoint. Consulta [x-mint extension](/es/api-playground/openapi-setup#x-mint-extension) para más detalles.
