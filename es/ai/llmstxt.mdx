...
La descripción de cada página proviene del campo `description` en su frontmatter. Las páginas sin un campo `description` aparecen en el archivo `llms.txt` sin descripción.
...
Tu archivo personalizado `llms.txt` o `llms-full.txt` debe incluir el título del sitio como encabezado H1. El resto del contenido es opcional. Consulta [Format](https://llmstxt.org/#format) en la especificación de `llms.txt` para obtener más información sobre secciones opcionales y mejores prácticas.