...
Configura las comprobaciones de CI habilitadas para un despliegue desde la página de [Complementos](https://dashboard.mintlify.com/products/addons) de tu panel. Activa las comprobaciones que quieras ejecutar.

Al habilitar comprobaciones, puedes elegir ejecutarlas en nivel `Warning` o `Blocking`.
...
Para ver los resultados de esta verificación, visita la página de resultados de comprobación de GitHub para un commit específico.
...
Mintlify permite ejecutar Vale automáticamente en una verificación de CI y mostrar los resultados como un estado de verificación.

<div id="configuration">
  #### Configuración
</div>

Si tienes un archivo `.vale.ini` en el directorio raíz de contenido de tu despliegue, usaremos automáticamente ese archivo de configuración. También usaremos automáticamente cualquier archivo de configuración en el `stylesPath` que especifiques.
...
Vale admite una variedad de [paquetes](https://vale.sh/docs/keys/packages) que pueden utilizarse para detectar errores de ortografía y de estilo.
...
Vale no es compatible con `MDX` de forma nativa, pero el autor de Vale ha proporcionado una [extensión personalizada](https://github.com/errata-ai/MDX) para habilitarlo.
