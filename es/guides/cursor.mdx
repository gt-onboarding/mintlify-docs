...
Las reglas de Cursor proporcionan contexto persistente sobre tu documentación, lo que garantiza sugerencias más consistentes y alineadas con tus estándares y estilo.

* **Reglas del proyecto**: se almacenan en el repositorio de tu documentación y se comparten con tu equipo.
* **Reglas del usuario**: se aplican a tu entorno personal de Cursor.

Recomendamos crear reglas del proyecto para tu documentación, de modo que todas las personas colaboradoras tengan acceso a las mismas reglas.

Crea archivos de reglas en el directorio `.cursor/rules` de tu repositorio de documentación. Consulta la [documentación de reglas de Cursor](https://docs.cursor.com/context/rules) para obtener instrucciones completas de configuración.

<div id="example-project-rule">
  ## Regla de ejemplo del proyecto
</div>

Esta regla le proporciona a Cursor el contexto necesario para formatear correctamente los Componentes (Mintlify) y seguir las mejores prácticas de redacción técnica.
...
Añade esta regla, con cualquier modificación, como un archivo `.mdc` en el directorio `.cursor/rules` del repositorio de tu documentación.
