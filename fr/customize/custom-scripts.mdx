...
Utilisez CSS pour mettre en forme les éléments HTML, ou ajoutez du CSS et du JavaScript personnalisés afin de personnaliser totalement l’apparence et le comportement de votre documentation.
...
Les valeurs arbitraires de Tailwind CSS ne sont pas prises en charge. Pour des valeurs personnalisées, utilisez plutôt la propriété `style`.
...
Ajoutez des fichiers CSS à votre dépôt et les classes qu’ils définissent seront appliquées et disponibles dans tous vos fichiers `MDX`.
...
Mintlify propose un ensemble d’identifiants et de sélecteurs communs pour vous aider à baliser les éléments importants de l’interface.

<Tip>
  Utilisez l’inspection d’élément (Inspecter l’élément) pour trouver les références des éléments que vous souhaitez personnaliser.
</Tip>
...
<Warning>
  Les références et le style des éléments courants sont susceptibles d’évoluer à mesure que la plateforme évolue. Veuillez utiliser les styles personnalisés avec prudence.
</Warning>
...
Le JavaScript personnalisé vous permet d’ajouter du code exécutable personnalisé à l’échelle du site. C’est l’équivalent d’ajouter une balise `<script>` contenant du code JavaScript à chaque page.
