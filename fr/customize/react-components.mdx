...
Pour importer des composants React dans vos fichiers `MDX`, les fichiers de composants doivent se trouver dans le dossier `snippets`. Vous pouvez ensuite les importer dans n’importe quelle page `MDX` de votre documentation. Pour en savoir plus, consultez les [extraits réutilisables](/fr/create/reusable-snippets).
...
Le générateur de couleurs se présente comme un composant React interactif.
...
<AccordionGroup>
  <Accordion title="Impact du rendu côté client">
    ...
    * **SEO** : les moteurs de recherche peuvent ne pas indexer complètement le contenu dynamique.
    * **Chargement initial** : les visiteurs peuvent voir un clignotement de contenu en cours de chargement avant l’affichage des composants.
  ...
  <Accordion title="Bonnes pratiques de performance">
    ...
    * **Réduire les re-rendus** : scindez les gros composants en plus petits pour éviter les re-rendus en cascade.
    * **Chargement différé (lazy loading)** : envisagez de différer le chargement des composants complexes pour améliorer le temps de chargement initial de la page.
  </Accordion>
</AccordionGroup>