...
La personnalisation adapte votre documentation à chaque utilisateur lorsqu’il est connecté. Par exemple, vous pouvez préremplir ses clés API, afficher du contenu propre à son offre ou à son rôle, ou masquer les sections auxquelles il n’a pas besoin d’accéder.

<div id="personalization-features">
  ## Fonctionnalités de personnalisation
</div>

Personnalisez votre contenu avec ces fonctionnalités de personnalisation.

<div id="api-key-prefilling">
  ### Préremplissage de la clé API
</div>

Renseignez automatiquement les champs du bac à sable API avec des valeurs propres à chaque utilisateur en renvoyant des noms de champs identiques à ceux utilisés dans vos données utilisateur. Pour que le préremplissage automatique fonctionne, les noms de champs de vos données utilisateur doivent correspondre exactement à ceux du bac à sable API.

<div id="dynamic-mdx-content">
  ### Contenu MDX dynamique
</div>

Affichez du contenu dynamique en fonction d’informations sur l’utilisateur, comme son nom, son forfait ou son organisation, en utilisant la variable `user`.
...
Limitez les pages visibles par vos utilisateurs en ajoutant des champs `groups` au frontmatter de vos pages. Par défaut, toutes les pages sont visibles par tous les utilisateurs.
...
<ParamField path="expiresAt" type="number">
  Heure d’expiration de la session en **secondes depuis l’époque Unix**. Si l’utilisateur charge une page après cette échéance, ses données stockées sont automatiquement supprimées et il doit se réauthentifier.
  <Warning><b>Pour les handshakes JWT :</b> Cela diffère de la revendication `exp` du JWT, qui détermine quand un JWT est considéré comme invalide. Pour des raisons de sécurité, définissez la revendication `exp` du JWT sur une courte durée (10 secondes ou moins). Utilisez `expiresAt` pour la durée réelle de la session (de quelques heures à plusieurs semaines).</Warning>
</ParamField>
...
<ParamField path="content" type="object">
  ...
  **Rendu conditionnel avancé** :
  ...
  <Note>
    Les informations dans `user` ne sont disponibles que pour les utilisateurs connectés. Pour les utilisateurs déconnectés, la valeur de `user` est `{}`. Pour éviter que la page ne plante pour les utilisateurs déconnectés, utilisez toujours l’opérateur d’enchaînement optionnel sur vos champs `user`. Par exemple, `{user.org?.plan}`.
  </Note>
</ParamField>

<ParamField path="apiPlaygroundInputs" type="object">
  Valeurs propres à l’utilisateur qui pré-remplissent les champs du Bac à sable API. Elles font gagner du temps en auto-complétant les données lors des tests d’API.
  ...
  **Exemple** :
  ...
  Si un utilisateur effectue des requêtes sur un sous-domaine spécifique, vous pouvez envoyer `{ server: { subdomain: 'foo' } }` comme champ `apiPlaygroundInputs`. Cette valeur sera pré-remplie sur toute page d’API comportant le champ `subdomain`.
...
```json
{
  ...
  "groups": ["admin", "utilisateurs-beta"],
  "content": {
    "firstName": "Jane",
    "lastName": "Smith",
    "company": "TechCorp",
...
Sélectionnez la méthode d’authentification par poignée de main que vous souhaitez configurer.
...
<Tabs>
  <Tab title="JWT">
    ...
    * Un service backend capable de créer des URL de redirection
    ...
    Votre documentation est hébergée sur `docs.foo.com`. Vous souhaitez séparer votre documentation de votre tableau de bord (ou vous n’en avez pas) et activer la personnalisation.
    ...
    * Signez le JWT et redirigez vers `https://docs.foo.com#{SIGNED_JWT}`.
    ...
    Pour rediriger les utilisateurs vers des sections spécifiques après la connexion, utilisez le format d’URL suivant : `https://docs.foo.com/page#jwt={SIGNED_JWT}&anchor={ANCHOR}`.
    ...
    * La capacité de créer un endpoint d’API accessible via des jetons d’accès OAuth
    ...
    <Steps>
      ...
      <Step title="Configure your OAuth personalization settings.">
        ...
        * **Scopes** : permissions à demander. Copiez la chaîne de scope **entière** (par exemple, pour un scope comme `provider.users.docs`, copiez l’intégralité de `provider.users.docs`). Doit correspondre aux scopes de l’endpoint que vous avez configuré à la première étape.
        * **Token URL** : votre endpoint d’échange de jetons OAuth.
        * **Info API URL** : endpoint permettant de récupérer les données utilisateur pour la personnalisation. Créé à la première étape.
  ...
  <Tab title="Session partagée">
    ...
    * Un tableau de bord ou portail utilisateur avec une authentification de session basée sur des cookies.
    * La possibilité de créer un endpoint d’API sur la même origine ou le même sous-domaine que votre tableau de bord.
    ...
    * Votre documentation est hébergée sur le même domaine ou sous-domaine que votre tableau de bord.
      * Si votre tableau de bord est sur `foo.com`, votre **documentation** doit être hébergée sur `foo.com` ou `*.foo.com`.
      * Si votre tableau de bord est sur `*.foo.com`, votre **documentation** doit être hébergée sur `foo.com` ou `*.foo.com`.

    ### Mise en œuvre

    <Steps>
      <Step title="Créer l’endpoint d’API des infos utilisateur">
        Créez un endpoint d’API qui :
      ...
      <Step title="Configurer vos paramètres de personnalisation">
        1. Dans votre tableau de bord, allez dans [Authentication](https://dashboard.mintlify.com/settings/deployment/authentication).
        2. Sélectionnez **Personnalisation**.
        3. Sélectionnez **Shared Session**.
        4. Saisissez votre **Info API URL**, c’est-à-dire l’endpoint de la première étape.
        5. Saisissez votre **Login URL**, la page où les utilisateurs se connectent à votre tableau de bord.
    ...
    #### Tableau de bord sur un sous-domaine, documentation sur un sous-domaine

    Vous avez un tableau de bord sur `dash.foo.com`, qui utilise une authentification de session basée sur des cookies. Vos routes d’API de tableau de bord sont hébergées sur `dash.foo.com/api`. Vous souhaitez configurer la personnalisation pour votre documentation hébergée sur `docs.foo.com`.
    ...
    **Processus de configuration** :
    ...
    #### Tableau de bord sur un sous-domaine, documentation à la racine

    Vous avez un tableau de bord sur `dash.foo.com`, qui utilise une authentification de session basée sur des cookies. Vos routes d’API de tableau de bord sont hébergées sur `dash.foo.com/api`. Vous souhaitez configurer la personnalisation pour votre documentation hébergée sur `foo.com/docs`.
    ...
    **Processus de configuration** :
    ...
    #### Tableau de bord à la racine, documentation à la racine

    Vous avez un tableau de bord sur `foo.com/dashboard`, qui utilise une authentification de session basée sur des cookies. Vos routes d’API de tableau de bord sont hébergées sur `foo.com/api`. Vous souhaitez configurer la personnalisation pour votre documentation hébergée sur `foo.com/docs`.
    ...
    **Processus de configuration** :
    ...
    <Note>
      Aucune configuration CORS n’est nécessaire puisque le tableau de bord et la documentation partagent le même domaine.
    </Note>
  </Tab>
</Tabs>