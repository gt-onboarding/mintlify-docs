---
title: "Vercel"
description: "Hébergez votre documentation sur un sous-chemin personnalisé avec Vercel"
---
...
Le fichier `vercel.json` configure la façon dont votre projet est construit et déployé. Il se trouve à la racine de votre projet et contrôle divers aspects de votre déploiement, notamment le routage, les redirections, les en-têtes et les paramètres de build.
...
Vous pouvez personnaliser le sous-chemin avec la valeur de votre choix, telle que `/docs`, `/help` ou `/guides`. Vous pouvez également utiliser des sous-chemins plus profondément imbriqués, comme `/product/docs`.
...
1. Accédez à [Configuration du domaine personnalisé](https://dashboard.mintlify.com/settings/deployment/custom-domain) dans votre tableau de bord.
2. Activez l’option **Héberger à `/docs`**.

<Frame>
  <img alt="Capture d’écran de la page Configuration du domaine personnalisé. L’option Héberger à `/docs` est activée et mise en évidence par un rectangle orange." src="/images/subpath/toggle-light.png" className="block dark:hidden" />

  <img alt="Capture d’écran de la page Configuration du domaine personnalisé. L’option Héberger à `/docs` est activée et mise en évidence par un rectangle orange." src="/images/subpath/toggle-dark.png" className="hidden dark:block" />
</Frame>
...
* **`destination`**: L’URL vers laquelle la requête doit être mise en proxy.
* **`:match*`**: Un caractère générique qui capture tous les segments de chemin après votre sous-chemin.

Pour en savoir plus, consultez [Configuration des projets avec vercel.json : Rewrites](https://vercel.com/docs/projects/project-configuration#rewrites) dans la documentation Vercel.
...
Pour utiliser un sous-chemin personnalisé (tout chemin autre que `/docs`), vous devez organiser les fichiers de votre documentation dans votre dépôt de façon à refléter la structure de ce sous-chemin. Par exemple, si votre documentation est accessible à l’adresse `yoursite.com/help`, les fichiers de documentation doivent se trouver dans un répertoire `help/`.

Utilisez le générateur ci-dessous pour créer votre configuration de réécritures. Ajoutez ces réécritures à votre fichier `vercel.json`.

<VercelJsonGenerator />

<div id="using-external-proxies-with-vercel">
  ## Utilisation de proxys externes avec Vercel
</div>

Si vous utilisez un proxy externe (comme Cloudflare ou AWS CloudFront) devant votre déploiement Vercel, vous devez le configurer correctement pour éviter tout conflit avec la vérification de domaine de Vercel et l’émission des certificats SSL.
...
Votre proxy externe doit autoriser le trafic vers ces chemins précis sans le bloquer, le rediriger ni le mettre fortement en cache :
