---
title: "Proxy inverse"
description: "Configurer un proxy inverse personnalisé pour servir votre documentation"
---

<Note>
  Les configurations de proxy inverse ne sont prises en charge que pour les [formules Custom](https://mintlify.com/pricing?ref=reverse-proxy).
</Note>

Pour servir votre documentation via un proxy inverse personnalisé, vous devez configurer des règles de routage, des stratégies de mise en cache et le transfert des en-têtes.

Lorsque vous déployez un proxy inverse, surveillez d’éventuels problèmes liés à la vérification de domaine, à l’émission des certificats SSL, aux flux d’authentification, aux performances et au suivi analytique.

<div id="routing-configuration">
  ## Configuration du routage
</div>

Proxiez ces chemins vers votre sous-domaine Mintlify avec les stratégies de mise en cache indiquées :
...
**Symptômes** : La documentation se charge, mais certaines fonctionnalités ne fonctionnent pas. Les appels d’API échouent.
...
**Symptômes** : Chargement des pages lent et décalages de mise en page.
