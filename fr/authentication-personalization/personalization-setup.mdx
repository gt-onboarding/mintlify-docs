...
Consultez la section [Format des données utilisateur](#user-data-format) ci-dessous pour des exemples détaillés et des recommandations de mise en œuvre.
...
Les utilisateurs ne verront que les pages dont les `groups` correspondent à ceux auxquels ils appartiennent.
...
<ParamField
  ...
  type="object"
>
  Valeurs propres à l’utilisateur qui préremplissent les champs du Bac à sable API. Elles font gagner du temps en remplissant automatiquement ses données lors des tests d’API.
...
<Tabs>
  <Tab title="JWT">
    ...
    * Un service backend capable de créer des URL de redirection
    ...
    <Steps>
      ...
      <Step title="Intégrez la personnalisation Mintlify dans votre flux de connexion.">
    ...
    Générez un secret JWT. Puis créez un endpoint de connexion à `https://foo.com/docs-login` qui initie un flux de connexion vers votre documentation.
  ...
  <Tab title="OAuth 2.0">
    ...
    * La possibilité de créer un endpoint d’API accessible via des jetons d’accès OAuth
    ...
    <Steps>
      <Step title="Créer un endpoint d’API d’informations utilisateur.">
        ...
        * Retourne les données utilisateur au format `User`. Voir la section [Format des données User](#user-data-format) ci-dessus pour plus d’informations.
    ...
    **Créez un endpoint d’informations utilisateur** à `api.foo.com/docs/user-info`, qui nécessite un jeton d’accès OAuth avec le scope `provider.users.docs` et répond avec les données personnalisées de l’utilisateur :
  ...
  <Tab title="Shared session">
    ...
    * Votre documentation est hébergée sur le même domaine ou sous-domaine que votre tableau de bord.
      * Si votre tableau de bord est à `foo.com`, votre **documentation** doit être hébergée à `foo.com` ou `*.foo.com`.
      * Si votre tableau de bord est à `*.foo.com`, votre **documentation** doit être hébergée à `foo.com` ou `*.foo.com`.

    ### Implémentation

    <Steps>
      <Step title="Créer un endpoint d’API d’infos utilisateur">
        Créez un endpoint d’API qui :

        * Utilise votre authentification de session existante pour identifier les utilisateurs
        * Retourne les données utilisateur au format `User` (voir la section [Format des données utilisateur](#user-data-format) ci-dessus)
    ...
    #### Tableau de bord sur sous-domaine, documentation sur sous-domaine

    Vous avez un tableau de bord à `dash.foo.com`, qui utilise une authentification de session basée sur des cookies. Les routes de l’API de votre tableau de bord sont hébergées à `dash.foo.com/api`. Vous souhaitez configurer la personnalisation pour votre documentation hébergée à `docs.foo.com`.

    **Processus de configuration** :

    1. **Créez l’endpoint** `dash.foo.com/api/docs/user-info` qui identifie les utilisateurs via l’authentification de session et répond avec leurs données utilisateur.
    ...
    #### Tableau de bord sur sous-domaine, documentation à la racine

    Vous avez un tableau de bord à `dash.foo.com`, qui utilise une authentification de session basée sur des cookies. Les routes de l’API de votre tableau de bord sont hébergées à `dash.foo.com/api`. Vous souhaitez configurer la personnalisation pour votre documentation hébergée à `foo.com/docs`.
    ...
    **Processus de configuration** :
    ...
    Vous avez un tableau de bord à `foo.com/dashboard`, qui utilise une authentification de session basée sur des cookies. Les routes de l’API de votre tableau de bord sont hébergées à `foo.com/api`. Vous souhaitez configurer la personnalisation pour votre documentation hébergée à `foo.com/docs`.

    **Processus de configuration** :

    1. **Créez l’endpoint** `foo.com/api/docs/user-info` qui identifie les utilisateurs via l’authentification de session et répond avec leurs données utilisateur.
