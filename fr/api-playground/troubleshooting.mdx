...
<AccordionGroup>
  <Accordion title="All of my OpenAPI pages are completely blank">
    ...
    Si la zone de texte qui s’affiche en dessous a une bordure verte, votre document a passé la validation.
    Il s’agit du même package de validation que celui utilisé par Mintlify pour valider les documents OpenAPI ; si votre document
  ...
  <Accordion title="One of my OpenAPI pages is completely blank">
    C’est généralement dû à une faute de frappe dans le champ `openapi` des métadonnées de la page. Assurez-vous que la méthode HTTP et le chemin correspondent exactement à la méthode HTTP et au chemin dans le document OpenAPI.

    Voici un exemple de ce qui peut poser problème :
    ...
    Un autre problème courant est un nom de fichier mal orthographié. Si vous spécifiez un document OpenAPI particulier dans le champ `openapi`, assurez-vous que le nom de fichier est correct. Par exemple, si vous avez deux documents OpenAPI `openapi/v1.json` et `openapi/v2.json`, vos métadonnées pourraient ressembler à ceci :
  ...
  <Accordion title="Requests from the API Playground don't work">
    Si vous utilisez un domaine personnalisé, le problème peut venir de votre reverse proxy. Par défaut, les requêtes effectuées via le Bac à sable API commencent par une requête `POST` vers le chemin `/_mintlify/api/request` sur le site de documentation. Si votre reverse proxy n’autorise que les requêtes `GET`, alors toutes ces requêtes échoueront. Pour corriger cela, configurez votre reverse proxy pour autoriser les requêtes `POST` vers le chemin `/_mintlify/api/request`.

    Sinon, si votre reverse proxy ne permet pas d’accepter des requêtes `POST`, vous pouvez configurer Mintlify pour envoyer les requêtes directement à votre backend avec le paramètre `api.playground.proxy` dans `docs.json`, comme décrit dans la [documentation des paramètres](/fr/settings#param-proxy). Avec cette configuration, vous devrez activer CORS sur votre serveur, puisque les requêtes proviendront directement des navigateurs des utilisateurs plutôt que de passer par votre proxy.
  </Accordion>
