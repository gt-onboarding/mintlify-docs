...
description: "Installez la CLI pour prévisualiser et développer vos docs en local"
icon: "terminal"
---
...
<Steps>
  ...
  <Step title="Prévisualiser en local.">
    Accédez à votre répertoire de documentation (emplacement de votre fichier `docs.json`) et exécutez la commande suivante :
...
Si l’aperçu local n’est pas synchronisé avec ce que vous voyez en production sur le web, mettez à jour votre CLI locale :

```bash
mint update
```

Si la commande `mint update` n’est pas disponible sur votre version locale, réinstallez la CLI avec la dernière version :
...
Par défaut, la CLI utilise le port 3000. Vous pouvez définir un autre port à l'aide de l’option `--port`. Pour exécuter la CLI sur le port 3333, par exemple, utilisez la commande suivante :
...
```mdx
Le port 3000 est déjà utilisé. Essai avec le port 3001 à la place.
```
...
Si vous utilisez l’authentification partielle pour restreindre l’accès à votre documentation, vous pouvez lancer un aperçu en tant que groupe d’authentification spécifique à l’aide de l’option `--group [groupname]`.
...
Bien que `mint dev` soit la commande la plus couramment utilisée, d’autres commandes sont disponibles pour gérer votre documentation.

<div id="finding-broken-links">
  ### Recherche de liens cassés
</div>

La CLI peut vous aider à valider les liens de référence présents dans votre documentation. Pour identifier les liens cassés, utilisez la commande suivante :
...
Vous pouvez utiliser la CLI pour vérifier votre fichier OpenAPI et détecter d’éventuelles erreurs avec la commande suivante :
...
<AccordionGroup>
  ...
  <Accordion title="mintlify vs. package mint">
    Si vous rencontrez des problèmes avec le package de la CLI, exécutez d’abord `npm ls -g`. Cette commande affiche les packages installés globalement sur votre machine.
