...
Le Model Context Protocol (MCP) est un protocole ouvert qui standardise les connexions entre les applications d’IA et des services externes, comme la documentation. Mintlify génère un serveur MCP à partir de votre documentation et de vos spécifications OpenAPI, afin de préparer votre contenu pour l’écosystème IA au sens large, où n’importe quel client MCP (comme Claude, Cursor, Goose, etc.) peut se connecter à votre documentation et à vos API.

Votre serveur MCP expose des outils permettant aux applications d’IA d’explorer votre documentation et d’interagir avec vos API.

<div id="accessing-your-mcp-server">
  ## Accès à votre serveur MCP
</div>

<Note>
  Les serveurs MCP ne peuvent être générés que pour une documentation publique. La documentation protégée par une authentification des utilisateurs finaux n’est pas accessible pour la génération de serveur.
</Note>

Mintlify génère automatiquement un serveur MCP pour votre documentation et l’héberge à l’URL de votre site de documentation avec le chemin `/mcp`. Par exemple, le serveur MCP de Mintlify est disponible à l’adresse `https://mintlify.com/docs/mcp`.
...
Tous les serveurs MCP incluent l’outil `search` par défaut, permettant aux utilisateurs d’interroger des informations depuis votre documentation dans d’autres outils.

Si vous avez une [offre Pro ou Enterprise](https://mintlify.com/pricing?ref=mcp), vous pouvez exposer des endpoints de votre spécification OpenAPI en tant qu’outils MCP.
  ...
  <Tab title="Claude">
    <Steps>
      ...
      <Step title="Publiez l’URL de votre serveur MCP pour vos utilisateurs.">
        ...
        4. Sélectionnez **Add**.
        5. En utilisant Claude, sélectionnez le bouton des pièces jointes (l’icône plus).
...
<Tabs>
  <Tab title="Menu contextuel">
    En haut de cette page, ouvrez le menu contextuel et choisissez **Connecter à Cursor** ou **Connecter à VS Code** pour relier le serveur MCP de Mintlify à l’IDE de votre choix.
  </Tab>
  ...
  <Tab title="Claude">
    ...
    Consultez la [documentation Model Context Protocol](https://modelcontextprotocol.io/docs/tutorials/use-remote-mcp-server#connecting-to-a-remote-mcp-server) pour plus d’informations.
  </Tab>
  ...
  <Tab title="Claude Code">
    ...
    Consultez la [documentation Claude Code](https://docs.anthropic.com/en/docs/claude-code/mcp#installing-mcp-servers) pour plus d’informations.
  </Tab>
  ...
  <Tab title="Cursor">
    ...
    Pour connecter le serveur MCP de Mintlify à Cursor, cliquez sur le bouton **Installer dans Cursor**. Pour le connecter manuellement, suivez ces étapes :
    ...
    Voir [Installing MCP servers](https://docs.cursor.com/en/context/mcp#installing-mcp-servers) dans la documentation de Cursor pour plus d’informations.
  </Tab>
  ...
  <Tab title="VS Code">
    ...
    Pour connecter le serveur MCP de Mintlify à VS Code, cliquez sur le bouton **Installer dans VS Code**. Pour le connecter manuellement, créez un fichier `.vscode/mcp.json` et ajoutez :
    ...
    Consultez la [documentation VS Code](https://code.visualstudio.com/docs/copilot/chat/mcp-servers) pour plus d’informations.
  </Tab>
</Tabs>
...
Lorsque vous activez un endpoint d’API pour MCP, le serveur inclut les exigences d’authentification définies dans vos `securitySchemes` et `securityRequirement` OpenAPI. Toutes les clés sont gérées directement par l’outil et ne sont ni stockées ni traitées par Mintlify.
...
<AccordionGroup>
  ...
  <Accordion title="Descriptions d’outils manquantes ou peu claires">
    ...
    3. Utilisez le tableau de bord MCP pour vérifier la façon dont vos endpoints apparaissent en tant qu’outils.
  </Accordion>
</AccordionGroup>