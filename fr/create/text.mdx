...
Vous pouvez vérifier la présence de liens cassés dans votre documentation à l’aide de la [CLI](/fr/installation) :
...
<div id="single-line-blockquotes">
  ### Blocquotes sur une ligne
</div>

Ajoutez `>` avant le texte pour créer un blocquote :
...
> Voici le premier paragraphe d’une citation multilignes.
>
> Voici le deuxième paragraphe, séparé par une ligne vide précédée de `>`.

<Tip>
  Utilisez les citations avec parcimonie pour préserver leur impact visuel et leur sens. Envisagez d’utiliser des [encadrés](/fr/components/callouts) pour les notes, les avertissements et autres informations.
</Tip>
...
Maîtrisez l’espacement et les retours à la ligne pour améliorer la lisibilité du contenu.
...
Utilisez les balises HTML `<br />` pour imposer des retours à la ligne au sein des paragraphes :
...
* Respectez l’ordre des titres (ne passez pas de H2 à H4)
...
* Utilisez le gras pour l’emphase, pas pour des paragraphes entiers
* Réservez l’italique aux termes, aux titres ou à une emphase subtile
* Évitez la sur‑mise en forme qui détourne l’attention du contenu
