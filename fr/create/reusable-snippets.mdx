...
Toute page présente dans le répertoire `snippets` sera traitée comme un extrait et ne sera pas
...
1. Ajoutez le contenu que vous souhaitez réutiliser à votre fichier de snippet. Vous pouvez éventuellement ajouter des variables qui seront renseignées via des props
   lors de l’import du snippet. Dans cet exemple, notre variable est word.

```typescript snippets/my-snippet.mdx
Hello world! Voici le contenu que je souhaite réutiliser sur plusieurs pages. 
```

2. Importez l’extrait de code dans votre fichier de destination.
...
1. Vous pouvez, si vous le souhaitez, ajouter des variables qui seront renseignées via des props lors de l’import de l’extrait. Dans cet exemple, notre variable est word.

```typescript snippets/my-snippet.mdx
Mon mot-clé du jour est {word}.
```

2. Importez l’extrait dans votre fichier de destination en utilisant la variable. La propriété sera renseignée selon vos spécifications.
...
1. Exportez une variable depuis votre fichier de snippets :
...
1. Exportez un composant JSX depuis votre fichier d’extrait. (Voir [Composants React](/fr/customize/react-components) pour plus d’informations) :
