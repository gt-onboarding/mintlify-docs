...
Le compteur est rendu comme un composant React interactif.
...
<AccordionGroup>
  <Accordion title="Impact du rendu côté client">
    Les composants basés sur des hooks React s’exécutent côté client, ce qui entraîne plusieurs implications :
  ...
  <Accordion title="Bonnes pratiques de performance">
    - **Optimiser les tableaux de dépendances** : N’incluez que les dépendances nécessaires dans les tableaux de dépendances de vos `useEffect`.
    - **Mémoriser les calculs complexes** : Utilisez `useMemo` ou `useCallback` pour les opérations coûteuses.
    - **Réduire les re-rendus** : Découpez les composants volumineux en plus petits pour éviter les re-rendus en cascade.
